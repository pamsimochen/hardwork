...
#define TASK_IP_RUN_PRIORITY 15 //10
 ...
 * @param NSF3_W              : Work memory for NSF3
 ...
 * @param Dct_Enable                 : Enabled/disabled the DCT
 * @param Nsf3_Enable                : Enabled/disabled the NSF3 (For OMAP5)
 */
/* ======================================================================= */
typedef struct {
 ...
 * @param Nsf3RuntimeParams         : NSF3 parameters
...
#define IP_RUN_Malloc(SIZE, BYTEALIGNMENT)      MSP_MallocExtn(SIZE, TIMM_OSAL_TRUE, BYTEALIGNMENT, 0/*TIMMOSAL_MEM_SEGMENT_EXT*/, NULL)
#define IP_RUN_Free(PTR)        if(PTR != NULL) MSP_Free(PTR)
#define IP_RUN_Memset(PTR, SIZE, VAL)   MSP_Memset(PTR, SIZE, VAL)
#define IP_RUN_Memcpy(PTR_Dst, PTR_Src, USIZE)   MSP_Memcpy(PTR_Dst, PTR_Src, USIZE)
 ...
 */
#define IP_RUN_PARAMCHECK(C, V)  if( !(C)) { \
        status = V;\
...
#define IP_RUN_EXIT_IF(_Cond, _ErrorCode) { \
        if((_Cond)) { \
            /*  status = _ErrorCode;*/ \
...
IP_RUN_RET_TYPE IP_RUN_Change_OCPTags(Uint16 SIMCOPDmaCtrl_TagCnt, IP_run *handle);
 ...
 */
/* ===========================================================================*/
IP_RUN_RET_TYPE IP_RUN_RegisterAlgo (IP_run *handle);
 ...
 */
/* ===========================================================================*/
IP_RUN_RET_TYPE  IP_RUN_UnRegisterAlgo (IP_run *handle);
...
IP_RUN_RET_TYPE IP_Run_Start(IP_run *handle);
