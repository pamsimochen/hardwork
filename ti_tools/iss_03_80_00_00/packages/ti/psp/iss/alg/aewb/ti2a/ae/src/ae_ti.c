 ...
 *  @file   ae_ti.c                                                  
 *                                                                    
 *  @path    /proj/vsi/users/venu/DM812x/IPNetCam_rel_1_8/ti_tools/iss_02_bkup/packages/ti/psp/iss/alg/aewb/ti2a/ae/src/                                                 
 *                                                                    
    ...
    IAEWB_Rgb *rgbData, XDAS_UInt8 *weight, void *customData)
{
    ...
    if(abs(curY * h->numHistory - h->avgY) < h->thrld * h->numHistory * 4 && h->avgY != (XDAS_UInt32)-1 ) {
        /* update avgY */
...
#ifdef AE_DEBUG_PRINTS
    dprintf("numHistory = %d, adjRatio = %lld targetBrightness:%d ,curY %d\n",
				h->numHistory, adjRatio, h->targetBrightness, curY);
#endif
...
#ifdef AE_DEBUG_PRINTS
            dprintf("we are here exposure time %d, %d, %d , %d\n", newExposureTime,
                    h->exposureTimeRange[i].max, h->exposureTimeRange[i].min, h->exposureTimeStepSize);
#endif
...
#ifdef AE_DEBUG_PRINTS
	    	dprintf("adjRatio, exposure = %lld, %d\n", adjRatio, newExposureTime);
#endif
		}
...
#ifdef AE_DEBUG_PRINTS
	    	dprintf("adjRatio, newApertureLevel = %lld, %d\n", adjRatio, newApertureLevel);
#endif
        }
