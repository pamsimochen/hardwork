...
typedef struct {
    ...
    volatile unsigned int serverFlags[2];                  // RESERVED, DO
                                                           // NOT USE
    volatile unsigned int clientFlags[2];                  // RESERVED, DO
                                                           // NOT USE
    volatile unsigned int reserved;                        // RESERVED, DO
                                                           // NOT USE

    volatile unsigned char serverLogBuf[REMOTE_DEBUG_LOG_BUF_SIZE];
    volatile unsigned char serverParamBuf[REMOTE_DEBUG_PARAM_BUF_SIZE]; // RESERVED, 
                                                                        // DO 
                                                                        // NOT 
                                                                        // USE
    volatile unsigned char clientParamBuf[REMOTE_DEBUG_PARAM_BUF_SIZE]; // RESERVED, 
                                                                        // DO 
                                                                        // NOT 
                                                                        // USE
