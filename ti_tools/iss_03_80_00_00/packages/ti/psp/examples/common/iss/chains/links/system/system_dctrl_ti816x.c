...
/* since CIG can be enable only on HDDAC path we need to alter the DC Config
 * when CIG is ON */
#define SWITCH_BP0_BP1
...
/* tied VENCs has some application problem when BP0/BP1 is switched hence
 * disabling it */
...
Int32 System_displayCtrlInit(UInt32 displayRes, UInt32 sdDisplayRes)
{
    ...
    memcpy(&gSystem_obj.displayCtrlCfg,
           &gSystem_dctrlTriDisplayConfig, sizeof(Vps_DcConfig));
