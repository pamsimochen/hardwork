...
Int32 CameraLink_drvCreateSensor(CameraLink_Obj * pObj, UInt16 instId)
{
    ...
    GT_assert(GT_DEFAULT_MASK, pInst->SensorHandle != NULL);
    return FVID2_SOK;
}
...
Int32 CameraLink_drvCreateInst(CameraLink_Obj * pObj, UInt16 instId)
{
    ...
    for (outId = 0; outId < pVipCreateArgs->numStream; outId++)
    {
        ...
        pVipOutPrm->pitch[0] =
            VpsUtils_align(pObj->maxWidth, ISS_BUFFER_ALIGNMENT * 2);
    ...
    GT_assert(GT_DEFAULT_MASK, pInst->cameraVipHandle != NULL);
                           ...
                           CameraLink_CreateParams * pPrm)
{
    ...
    {
        pObj->tmpBufAddr = VpsUtils_memAlloc(CAMERA_LINK_TMP_BUF_SIZE, 32);
    }
...
Int32 CameraLink_drvDelete(CameraLink_Obj * pObj)
{
    ...
    if (CAMERA_LINK_TMP_BUF_SIZE)
    {
        VpsUtils_memFree(pObj->tmpBufAddr, CAMERA_LINK_TMP_BUF_SIZE);
    }
