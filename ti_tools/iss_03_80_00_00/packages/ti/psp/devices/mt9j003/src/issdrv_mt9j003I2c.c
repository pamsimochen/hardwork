...
Int32 Iss_Mt9j003I2cRead8(Iss_Mt9j003Obj * pObj,
...
#ifdef ISS_MT9J003_USE_REG_CACHE
    UInt32 reg, port;
...
Int32 Iss_Mt9j003I2cWrite8(Iss_Mt9j003Obj * pObj,
                           UInt32 i2cInstId, UInt32 i2cDevAddr,
                           const UInt8 * regAddr,
                           UInt8 * regValue, UInt32 numRegs)
{
...
/* ===================================================================
 *  @func     Iss_Mt9j003UpdateRegCache
 *
 *  @desc     Function does the following
 *
 *  @modif    This function modifies the following structures
 ...
 *  ==================================================================
 */
Int32 Iss_Mt9j003UpdateRegCache(Iss_Mt9j003Obj * pObj)
{
    Int32 status = FVID2_SOK;
    ...
    devAddr[1] = pCreateArgs->deviceI2cAddr[0] + ISS_MT9J003_I2C_PORT1_OFFSET;
 ...
 * MT9J003 and then setup in auto reset mode - Select default TMDS core */
/* ===================================================================
...
Int32 Iss_Mt9j003Reset(Iss_Mt9j003Obj * pObj)
{
     ...
     * Normal operation
     */
    status = Iss_Mt9j003PowerDown(pObj, FALSE);
    if (status != FVID2_SOK)
        return status;
    ...
    regAddr[numRegs] = ISS_MT9J003_REG_SW_RST_0;
    ...
    status = Iss_Mt9j003UpdateRegCache(pObj);
    if (status != FVID2_SOK)
        return status;
...
Int32 Iss_Mt9j003PowerDown(Iss_Mt9j003Obj * pObj, UInt32 powerDown)
{
    ...
    numRegs = 0;
    regAddr[numRegs] = ISS_MT9J003_REG_SYS_CTRL_1;
    regValue[numRegs] = 0;
    numRegs++;
...
Int32 Iss_Mt9j003OutputEnable(Iss_Mt9j003Obj * pObj, UInt32 enable)
{
    ...
    devAddr = pCreateArgs->deviceI2cAddr[0] + ISS_MT9J003_I2C_PORT1_OFFSET;

    numRegs = 0;
    regAddr[numRegs] = ISS_MT9J003_REG_SYS_PWR_DWN_2;
    ...
    regAddr[numRegs] = ISS_MT9J003_REG_SYS_PWR_DWN;
...
Int32 Iss_Mt9j003Stop(Iss_Mt9j003Obj * pObj)
{
    ...
    //status = Iss_Mt9j003OutputEnable ( pObj, FALSE );
...
Int32 Iss_Mt9j003Start(Iss_Mt9j003Obj * pObj)
{
    ...
    //status = Iss_Mt9j003OutputEnable ( pObj, TRUE );
                           ...
                           Iss_SensorChipIdStatus * pStatus)
{
    ...
    regAddr[numRegs] = ISS_MT9J003_REG_VND_IDL;
    regValue[numRegs] = 0;
    numRegs++;

    regAddr[numRegs] = ISS_MT9J003_REG_VND_IDH;
    regValue[numRegs] = 0;
    numRegs++;

    regAddr[numRegs] = ISS_MT9J003_REG_DEV_IDL;
    regValue[numRegs] = 0;
    numRegs++;

    regAddr[numRegs] = ISS_MT9J003_REG_DEV_IDH;
    regValue[numRegs] = 0;
    numRegs++;

    regAddr[numRegs] = ISS_MT9J003_REG_DEV_REV;
    regValue[numRegs] = 0;
    numRegs++;
