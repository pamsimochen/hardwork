...
Int32 Iss_Ov10630I2cRead8 ( Iss_Ov10630Obj * pObj,
                        ...
                        UInt8 * regValue, UInt32 numRegs )
{
    ...
    #ifdef ISS_OV10630_USE_REG_CACHE
    UInt32 reg, port;
...
Int32 Iss_Ov10630I2cWrite8 ( Iss_Ov10630Obj * pObj,
                        UInt32 i2cInstId, UInt32 i2cDevAddr,
                        const UInt8 * regAddr,
                        UInt8 * regValue, UInt32 numRegs )
{
...
/* ===================================================================
 *  @func     Iss_Ov10630UpdateRegCache                                               
 *                                                                    
 *  @desc     Function does the following                             
 *                                                                    
 *  @modif    This function modifies the following structures         
 ...
 *  ==================================================================
 */        
Int32 Iss_Ov10630UpdateRegCache(Iss_Ov10630Obj * pObj)
{
    Int32 status = FVID2_SOK;
    ...
    devAddr[1] = pCreateArgs->deviceI2cAddr[0] + ISS_OV10630_I2C_PORT1_OFFSET;
 ...
 * OV10630 and then setup in auto reset mode - Select default TMDS core */
/* ===================================================================
...
Int32 Iss_Ov10630Reset ( Iss_Ov10630Obj * pObj )
{
     ...
     * Normal operation
     */
    status = Iss_Ov10630PowerDown ( pObj, FALSE );
    if ( status != FVID2_SOK )
        return status;
    ...
    regAddr[numRegs] = ISS_OV10630_REG_SW_RST_0;
    regValue[numRegs] = 0x2F;            /* reset AAC,
                                                            * HDCP, ACR,
                                                            * audio FIFO, SW */
    numRegs++;

    status = Iss_Ov10630I2cWrite8 ( pObj, pCreateArgs->deviceI2cInstId,
    ...
    status = Iss_Ov10630UpdateRegCache(pObj);
    if ( status != FVID2_SOK )
        return status;
...
Int32 Iss_Ov10630PowerDown ( Iss_Ov10630Obj * pObj, UInt32 powerDown )
{
    ...
    numRegs = 0;
    regAddr[numRegs] = ISS_OV10630_REG_SYS_CTRL_1;
    regValue[numRegs] = 0;
    numRegs++;

    status = Iss_Ov10630I2cRead8 ( pObj,
...
Int32 Iss_Ov10630OutputEnable ( Iss_Ov10630Obj * pObj, UInt32 enable )
{
    ...
    devAddr = pCreateArgs->deviceI2cAddr[0] + ISS_OV10630_I2C_PORT1_OFFSET;

    numRegs = 0;
    regAddr[numRegs] = ISS_OV10630_REG_SYS_PWR_DWN_2;
    regValue[numRegs] = 0xC3;                               /* Enable all
                                                            * expect outputs */
    if ( enable )
        regValue[numRegs] |= ( 1 << 2 );    /* Enable outputs  */

    regAddr[numRegs] = ISS_OV10630_REG_SYS_PWR_DWN;
...
Int32 Iss_Ov10630Stop ( Iss_Ov10630Obj * pObj )
{
 ...
 //   status = Iss_Ov10630OutputEnable ( pObj, FALSE );
