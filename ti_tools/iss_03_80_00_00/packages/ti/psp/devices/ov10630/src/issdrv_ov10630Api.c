...
#include <ti/psp/devices/ov10630/src/issdrv_ov10630Priv.h>
#include <ti/psp/devices/ov10630/issdrv_ov10630_config.h>
...
/* Global object storing all information related to all
  OV10630 driver handles */
static Iss_Ov10630CommonObj 	gIss_Ov10630CommonObj;

static I2c_Ctrl 				gOv10630_I2c_ctrl;
 ...
 * 
 * handle - OV10630 driver handle cmd - command cmdArgs - command arguments
...
int Transplant_DRV_imgsSetEshutter(Uint32 eshutterInUsec, int setRegDirect)
{
  ...
  int regValue2=0; 
  Int32 devAddr;
  gOv10630_I2c_ctrl.i2cInstId = Iss_platformGetI2cInstId();
  devAddr = OV_10630_ADDR;
 ...
 status=Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,regAddr, regValueSend, i);
...
{
  	Uint16 *regAddr=&gOv10630_I2c_ctrl.regAddr[0];
  	Uint8 *regValue=&gOv10630_I2c_ctrl.regValue[0];
 	int status, i,j;
  	Int32 delay = 100000;
  	Int32 devAddr;
	...
	*********************************************************************/
	...
	//DBG_PrintforTest(gOv10630_I2c_ctrl.i2cInstId,"gOv10630_I2c_ctrl.i2cInstId");	
	//DBG_PrintforTest(devAddr,"OV_10630_ADDR");
	...
	gOv10630_I2c_ctrl.numRegs = 1;
	for(j=0; j<i; j++)
	{

		//regValue[i]=(regValue[i]<<8)&0xff00;	
		//regValue[i]=regValue[i]&0x00ff;		
		//status = Iss_deviceWrite16(gOv10630_I2c_ctrl.i2cInstId,devAddr,&regAddr[i],&regValue[i],gOv10630_I2c_ctrl.numRegs);
		status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&regAddr[j],&regValue[j],gOv10630_I2c_ctrl.numRegs);
	...
	*/
	...
	//regAddr[i]	= PLL_CTRL01;			regValue[i] = 0x20 ;	i++;
		...
		status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&regAddr[i],&regValue[i],1);	
	/*		
	Uint16 MyregAddr[i];
	...
	//status = Iss_deviceRead16(gOv10630_I2c_ctrl.i2cInstId,devAddr,&MyregAddr[0],&MyregVal[0],1);
	//status = Iss_deviceRead16(gOv10630_I2c_ctrl.i2cInstId,devAddr,&MyregAddr[1],&MyregVal[1],1);
	Iss_device16AddRead8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&MyregAddr[0],&MyregVal[0],1);
	Iss_device16AddRead8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&MyregAddr[1],&MyregVal[1],1);
	...
	for(j=0; j<i; j++)
	{
		...
		//regValue[i]=regValue[i]&0x00ff;		
		//status = Iss_deviceWrite16(gOv10630_I2c_ctrl.i2cInstId,devAddr,&regAddr[i],&regValue[i],gOv10630_I2c_ctrl.numRegs);
		//status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&regAddr[i],&regValue[i],gOv10630_I2c_ctrl.numRegs);
		Iss_device16AddRead8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&MyregAddr[j],&MyregVal[j],1);		
...
{
    Iss_Ov10630Obj *pObj = ( Iss_Ov10630Obj * ) handle;
    Int32 status;
     ...
     * lock handle
     */
    Iss_Ov10630LockObj ( pObj );

    switch ( cmd )
    {
        case FVID2_START:
       //     status = Iss_Ov10630Start ( pObj );
            break;

        case FVID2_STOP:
      //      status = Iss_Ov10630Stop ( pObj );
            break;

        case IOCTL_ISS_SENSOR_GET_CHIP_ID:
      //      status = Iss_Ov10630GetChipId ( pObj, cmdArgs, cmdStatusArgs );
            break;

        case IOCTL_ISS_SENSOR_RESET:
      //      status = Iss_Ov10630Reset ( pObj );
            break;

        case IOCTL_ISS_SENSOR_REG_WRITE:
            status = Iss_Ov10630RegWrite ( pObj, cmdArgs );
            break;

        case IOCTL_ISS_SENSOR_REG_READ:
            status = Iss_Ov10630RegRead ( pObj, cmdArgs );
            break;

        case IOCTL_ISS_SENSOR_UPDATE_EXP_GAIN:
             status = Iss_Ov10630UpdateExpGain ( pObj, cmdArgs);
             break;

		case IOCTL_ISS_SENSOR_UPDATE_ITT:
          //   status = Iss_Ov10630UpdateItt ( pObj, cmdArgs);
             break;
 ...
 * 
 * This API does not configure the OV10630 is any way.
 * 
 ...
 * OV10630
 * 
 * drvId - driver ID, must be FVID2_ISS_SENSOR_OV10630_DRV instanceId - must 
                                ...
                                const FVID2_DrvCbParams * fdmCbParams )
{
    ...
    if ( drvId != FVID2_ISS_SENSOR_OV10630_DRV
...
Int32 Iss_Ov10630Delete ( Fdrv_Handle handle, Ptr deleteArgs )
{
     ...
     */
    Iss_Ov10630FreeObj ( pObj );

    return FVID2_SOK;
}

/* 
 * System init for OV10630 driver
 * 
...
Int32 Iss_Ov10630Init (  )
{
     ...
     */
    memset ( &gIss_Ov10630CommonObj, 0, sizeof ( gIss_Ov10630CommonObj ) );

    /*
     * Create global OV10630 lock
     */
    Semaphore_Params_init ( &semParams );

    semParams.mode = Semaphore_Mode_BINARY;

    gIss_Ov10630CommonObj.lock = Semaphore_create ( 1u, &semParams, NULL );

    if ( gIss_Ov10630CommonObj.lock == NULL )
        status = FVID2_EALLOC;

    if ( status == FVID2_SOK )
    {
        /*
         * Register OV10630 driver with FVID2 sub-system
         */
		gOv10630_I2c_ctrl.i2cInstId = Iss_platformGetI2cInstId();
		Vps_printf("\ngOv10630_I2c_ctrl.i2cInstId is %d\n",gOv10630_I2c_ctrl.i2cInstId);
		devAddr = OV_10630_ADDR;

		for(count=0; count< REGISTERNUMBER/*122*/; count++){
			gOv10630_I2c_ctrl.regAddr[count] = SensorConfigScript_yuv[count][0];
			gOv10630_I2c_ctrl.regValue[count] = SensorConfigScript_yuv[count][2];
			gOv10630_I2c_ctrl.numRegs = 1;

			if(SensorConfigScript_yuv[count][1] == 2){
				status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,
						&gOv10630_I2c_ctrl.regAddr[count],&gOv10630_I2c_ctrl.regValue[count],
						gOv10630_I2c_ctrl.numRegs);

                {
                    UInt8 value = 0;
                    status = Iss_device16AddRead8(gOv10630_I2c_ctrl.i2cInstId,devAddr,
						&gOv10630_I2c_ctrl.regAddr[count],&value,gOv10630_I2c_ctrl.numRegs);
...
#if 0 //dm385 evm		
		gOv10630_I2c_ctrl.regAddr[0] = 0x80;
		gOv10630_I2c_ctrl.regValue[0] = 0x00;
		gOv10630_I2c_ctrl.numRegs = 1;
		Iss_deviceWrite8(gOv10630_I2c_ctrl.i2cInstId,0x20,&gOv10630_I2c_ctrl.regAddr[0],&gOv10630_I2c_ctrl.regValue[0],gOv10630_I2c_ctrl.numRegs);
#endif		
		

        gIss_Ov10630CommonObj.fvidDrvOps.create = ( FVID2_DrvCreate ) Iss_Ov10630Create;
        gIss_Ov10630CommonObj.fvidDrvOps.delete = Iss_Ov10630Delete;
        gIss_Ov10630CommonObj.fvidDrvOps.control = Iss_Ov10630Control;
        gIss_Ov10630CommonObj.fvidDrvOps.queue = NULL;
        gIss_Ov10630CommonObj.fvidDrvOps.dequeue = NULL;
        gIss_Ov10630CommonObj.fvidDrvOps.processFrames = NULL;
        gIss_Ov10630CommonObj.fvidDrvOps.getProcessedFrames = NULL;
        gIss_Ov10630CommonObj.fvidDrvOps.drvId = FVID2_ISS_SENSOR_OV10630_DRV;

		Vps_printf("ov10630 FVID2_registerDriver start\n");
        status = FVID2_registerDriver ( &gIss_Ov10630CommonObj.fvidDrvOps );
		Vps_printf("ov10630 FVID2_registerDriver end\n");
        ...
        if ( status != FVID2_SOK )
        {
             ...
             */
            Semaphore_delete ( &gIss_Ov10630CommonObj.lock );
        }
    }
...
/* 
 * System de-init for OV10630 driver
 * 
...
Int32 Iss_Ov10630DeInit (  )
{
     ...
     */
    FVID2_unRegisterDriver ( &gIss_Ov10630CommonObj.fvidDrvOps );
...
{
    Semaphore_pend ( gIss_Ov10630CommonObj.lock, BIOS_WAIT_FOREVER );
...
{
    Semaphore_post ( gIss_Ov10630CommonObj.lock );
...
Iss_Ov10630Obj *Iss_Ov10630AllocObj (  )
{
    ...
    for ( handleId = 0; handleId < ISS_DEVICE_MAX_HANDLES; handleId++ )
    {
        ...
        pObj = &gIss_Ov10630CommonObj.Ov10630Obj[handleId];

        if ( pObj->state == ISS_OV10630_OBJ_STATE_UNUSED )
        {
            /*
             * free object found
             */
             ...
             */
            pObj->state = ISS_OV10630_OBJ_STATE_IDLE;
            ...
            if ( pObj->lock == NULL )
            {
                 ...
                 */
                found = FALSE;
                pObj->state = ISS_OV10630_OBJ_STATE_UNUSED;
            }
...
Int32 Iss_Ov10630UpdateExpGain ( Iss_Ov10630Obj * pObj, Ptr createArgs )
{
  ...
  //      Vps_printf("Exp gain: %d Exp: %d",ti2a_output_params.sensorGain,ti2a_output_params.sensorExposure);
	...
	g_int = OV10630_GainTableMap(ti2a_output_params.sensorGain);
    ...
    Iss_Ov10630Lock (  );
    {
    	extern Int32 aewbVendor;
			...
			{
				gOv10630_I2c_ctrl.regAddr[count] = 0x10;
				gOv10630_I2c_ctrl.regValue[count] = (c_int & 0xFF);
				gOv10630_I2c_ctrl.numRegs = 1;
				//Rfile_printf("sensor Exposure Time reg 0 value   :  < %5d, %5d > \n",gOv10630_I2c_ctrl.regValue[count],c_int );
				status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&gOv10630_I2c_ctrl.regAddr[count],
									&gOv10630_I2c_ctrl.regValue[count],gOv10630_I2c_ctrl.numRegs);

				gOv10630_I2c_ctrl.regAddr[count] = 0xF;
				gOv10630_I2c_ctrl.regValue[count] = ((c_int >> 8) & (0xFF));
				gOv10630_I2c_ctrl.numRegs = 1;
		    ...
		    if (ti2a_output_params.mask) 
			{
				gOv10630_I2c_ctrl.regAddr[count] = 0x0;
				gOv10630_I2c_ctrl.regValue[count] = g_int;
				gOv10630_I2c_ctrl.numRegs = 1;

			    //Rfile_printf("sensor Analog gain reg value  :  < 0x%2x > \n",gOv10630_I2c_ctrl.regValue[count]  );
				status = Iss_device16AddWrite8(gOv10630_I2c_ctrl.i2cInstId,devAddr,&gOv10630_I2c_ctrl.regAddr[count],
									&gOv10630_I2c_ctrl.regValue[count],gOv10630_I2c_ctrl.numRegs);
			}
		}
...
/*
  Update ITT Values
*/
Int32 Iss_Ov10630UpdateItt ( Iss_Ov10630Obj * pObj, Ptr createArgs )
{
	int status,devAddr,count  = 0;
	I2c_Ctrl 				ov10630_I2c_ctrl;

	ov10630_I2c_ctrl.i2cInstId = Iss_platformGetI2cInstId();
	devAddr = OV_10630_ADDR;
	if(gItt_RegisterParams.Control == 1){
	ov10630_I2c_ctrl.regAddr[count]  = gItt_RegisterParams.regAddr;
	ov10630_I2c_ctrl.regValue[count] = gItt_RegisterParams.regValue;
	ov10630_I2c_ctrl.numRegs = 1;

	status = Iss_deviceWrite8(ov10630_I2c_ctrl.i2cInstId,devAddr,&ov10630_I2c_ctrl.regAddr[count],&ov10630_I2c_ctrl.regValue[count],ov10630_I2c_ctrl.numRegs);
	}
	else if(gItt_RegisterParams.Control == 0){

	ov10630_I2c_ctrl.regAddr[count]  = gItt_RegisterParams.regAddr;
	ov10630_I2c_ctrl.regValue[count] = 0;
	ov10630_I2c_ctrl.numRegs = 1;
	status = Iss_deviceRead8(ov10630_I2c_ctrl.i2cInstId,devAddr,&ov10630_I2c_ctrl.regAddr[count],&ov10630_I2c_ctrl.regValue[count],ov10630_I2c_ctrl.numRegs);
	count  = 0;
	gItt_RegisterParams.regValue = ov10630_I2c_ctrl.regValue[count];
	}
	return status;
}
#endif
