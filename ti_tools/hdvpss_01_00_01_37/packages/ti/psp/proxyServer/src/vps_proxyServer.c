...
/**
 *  \file vps_proxyServer.c
 *
 *  \brief This file implements the functions / tasks that constitute VPS
 *         Proxy Server
 *
 *         Proxy Server provides a RPC for HDVPSS sub-system. Applications
 ...
 *         where VPS sub-system is hosted)
 */

//#define ENABLE_AUTO_SC_FOR_YUV
/**< Define this to enable auto-scaling and color-conversion when input is YUV420
    This is giving some problem when M3 code is placed in Cache write back area, hence
    Disabling this for now.
    DVR-RDK does not use this feature in any case
*/

#define SYSLINKS_IPC_NOTIFY_INCLUDED
/**< Configures Proxy Server to use IPC Notify for communication. This should
     ALWAYS be defined. */
#define VPS_PS_TASK_4_CB
/**< Configures Proxy Server to use a TASK to send notifications back to
     clients. This macro should ALWAYS be defined. */
...
#include <ti/psp/proxyServer/src/vps_proxyServerPriv.h>
...
/*                             Global Variables                               */
/* ========================================================================== */
static VPS_PSrvTaskObject gPSrvTaskObjInstances [VPS_PSRV_NUMBER_OF_TASKS];
...
static VPS_PSrvNtyNumsPerProcInst gPSrvNtyNo4AllCores[VPS_PSRV_MAX_NO_OF_CORES];
...
static Ptr gPSrvQSpace[VPS_PSRV_NUMBER_OF_TASKS][VPS_PSRV_TASK_MAX_Q_DEPTH];
/**< Place holder for Q of all the tasks, allocates the space for max depth */

static UInt8
   gPSrvTskStackSpace[VPS_PSRV_NUMBER_OF_TASKS][VPS_PSRV_TASK_STACK_DEPTH];
/**< Place holder for the tasks stacks */

#ifdef VPS_PS_TASK_4_CB
static VPS_PSrvTaskObject gPSrvNtfyClientTaskObj;
/**< Instance of task which handles notification to clients on completion of IO
     */
static Ptr gPSrvNtfyClientQSpace[VPS_PSRV_NTFY_TASK_MAX_Q_DEPTH];
/**< Place holder for Q of all the tasks, allocates the space for max depth */

static UInt8 gPSrvNtfyClientTaskStackSpace[VPS_PSRV_TASK_STACK_DEPTH];
/**< Place holder for the tasks stacks */
#endif /* VPS_PS_TASK_4_CB */

/* ========================================================================== */
/*                    Local Functions prototype                               */
/* ========================================================================== */
static Int32 psValidateInitParams(VPS_PSrvInitParams *pValInitParams);
...
static Int32 errorCb(FVID2_Handle handle,Ptr appData,Ptr errData,Ptr reserved);
...
static Int32 initSysLnkNtyShm(VPS_PSrvInitParams *pSysLnkNtyInitParams);
/**< Function to initialize sysLink with user supplied params */
static Void createFvid2Stream(
                VPS_PSrvFvid2CreateParams *creatParam,
                Uint32  remoteProcId);
/**< Function to create an FVID 2 stream */
static  Int32 psCallFvid2Api(
            Void *simplexCommand,
            VPS_PSrvTaskObject *pSrvTsk,
            UInt32 procId,
            UInt32 eventNo);
/**< Function to convert remote agent / client command to a FVID2 call */
static Int32 initializeClientNtfyTask(VPS_PSrvInitParams *pSysLnkNtyInitParams);
 ...
 *        host system once.
 *
 * \param   psInitParams   [IN] Pointer of type VPS_PSrvInitParams
 *
...
Int32  VPS_PSrvInit(VPS_PSrvInitParams *pPsInitParams)
{
    ...
    UInt32              index               =   0x0;
    Uint32              counter             =   VPS_PSRV_TASK_MAX_Q_DEPTH + 1u;
    VPS_PSrvTaskObject  *pPSrvTskInstance   =   NULL;
    Semaphore_Params    semParams;
    Task_Params         taskParams;
    static Uint32       state               =   VPS_PSRV_UNINITIALIAZED;

    GT_0trace(PSTrace, GT_ENTER, "PS: >>>>VPS_PSrvInit\n");
    ...
    if ((state == VPS_PSRV_UNINITIALIAZED) &&
        (psInitRtnVal == VPS_SOK))
    {
        ...
        VpsUtils_memset(
            ...
            0x0,
            sizeof(gPSrvTaskObjInstances));
        /* Notify number are initialized to 0xFF, during de-alloc on error
           we do not keep track of list of allocated resources.
           We assume all resources were allocated and blindly release.
           0x0 would be a valid notify number. 255 is highly unlikely */
         ...
         *  Create the task
         */
        for (index = 0x0; index < VPS_PSRV_NUMBER_OF_TASKS; index++)
        {
            pPSrvTskInstance               = &(gPSrvTaskObjInstances[index]);
            counter                        = VPS_PSRV_TASK_MAX_Q_DEPTH;

           /* Initialize semaphores for all tasks */
           Semaphore_Params_init(&semParams);
           pPSrvTskInstance->tSem = Semaphore_create(
            ...
            */
           psInitRtnVal =  VpsUtils_queCreate(
                               &(pPSrvTskInstance->spaceHolder4QHnld),
                               VPS_PSRV_TASK_MAX_Q_DEPTH,
            ...
            taskParams.stackSize        = VPS_PSRV_TASK_STACK_DEPTH;
            taskParams.arg0             = (UArg)pPSrvTskInstance;

            pPSrvTskInstance->taskInstance  = (VPS_PSrvHostTaskType)index;

            if (index == 0x00)
            {
                pPSrvTskInstance->taskOwnHndl   =   Task_create(
                                                        psCtrlTask,
                                                        &taskParams,
                                                        NULL);
            }
        ...
        /* Acquire a semaphore to guard the structure gPSrvNtyNo4AllCores used
           to store notification numbers for all the cores.
           Done here to ensure easy error handling */
        Semaphore_Params_init(&semParams);
        ...
        } /* Error check - release acquired resources */
        else
        {
 ...
 *        host configurations.
 *
 * \param   pValInitParams   [IN] Pointer of type VPS_PSrvInitParams
 *
 * \return  VPS_SOK if successful, else suitable error code
 */
...
static Int32 psValidateInitParams(VPS_PSrvInitParams *pValInitParams)
{
    ...
    if( (pValInitParams->sysLnkNoOfCores >= VPS_PSRV_MAX_NO_OF_CORES) ||
    ...
    {
        if ((pValInitParams->sysLnkNoOfNtyEvt[index] >= VPS_PSRV_MAX_NO_NOTIFY)
            ||
            (pValInitParams->sysLnkNoOfNtyEvt[index] < 1u))
        {
            ...
            GT_0trace(PSTrace, GT_ERR, "PS : VPS_PSRV_MAX_NO_NOTIFY \n");
            GT_1trace(PSTrace, GT_ERR, "PS : for core %d\n", index);
            valRtnVal = VPS_EBADARGS;
            break;
        }
        /* Check if any of the notify events is reserved notify event, failure
           to do so, could throttle control task */
...
static Void psCtrlTask(UInt32 taskArgs, UInt32 taskArgsUnused)
{
    VPS_PSrvTaskObject      *pPSrvCtrlTask =   NULL;
    VPS_PSrvCommandStruc    *pCmdStruct;
    Int32                   rtnVal;

    GT_assert( GT_DEFAULT_MASK, taskArgs != NULL);

    pPSrvCtrlTask   =   (VPS_PSrvTaskObject *)taskArgs;
    ...
    while (TRUE)
    {
        rtnVal      =   0x0;
        pCmdStruct  =   NULL;
        /* Loop required, on errors we would break out first loop. Providing a
           clean exit*/
        while (TRUE)
        {
            ...
            if ((Semaphore_pend (pPSrvCtrlTask->tSem, BIOS_WAIT_FOREVER )) == FALSE)
            {
            ...
            rtnVal  =   VpsUtils_queGet(
                            pPSrvCtrlTask->cmdQHndl,
                            (Ptr *)(&pCmdStruct),
                            0x01u,
                            BIOS_NO_WAIT);
            ...
            if (pCmdStruct->cmdType == VPS_FVID2_CMDTYPE_SIMPLEX)
            {
                VPS_PSrvFvid2CmdOverlay *cmdPtr =
                    (VPS_PSrvFvid2CmdOverlay *)pCmdStruct->simplexCmdArg;
                ...
                if (cmdPtr->command == VPS_FVID2_CREATE)
                {
                    ...
                    createFvid2Stream(
                        (VPS_PSrvFvid2CreateParams *)pCmdStruct->simplexCmdArg,
                        pCmdStruct->sysLnkProcId);
                    break;
                }
                else if (cmdPtr->command == VPS_FVID2_DE_INIT)
                {
                    VPS_PSrvFvid2DeInitParams *deInitParams =
                        (VPS_PSrvFvid2DeInitParams *)pCmdStruct->simplexCmdArg;
                    ...
                    GT_1trace(PSTrace, GT_INFO, "PS : [Done] - with %d\n",
                        deInitParams->returnValue);
                    break;
                } /* Check for create / delete / DeInit / Init */
                else if (cmdPtr->command == VPS_FVID2_INIT)
                {
                    VPS_PSrvFvid2InitParams *initParams =
                        (VPS_PSrvFvid2InitParams *)pCmdStruct->simplexCmdArg;
                ...
                {
                    if (cmdPtr->command == VPS_FVID2_GET_FIRMWARE_VERSION)
                    {
                        VPS_PSrvGetStatusVerCmdParams *verParams =
            ...
            } /* End of simplex command execution */
        } /* Error catcher loop */
    } /* Main infinite loop */
}
...
static Void psGenericTask(UInt32 taskArgs, UInt32 taskArgsUnused)
{
    ...
    VPS_PSrvTaskObject      *pPSrvIoTask    =   NULL;
    VPS_PSrvCommandStruc    *pIoCmdStruct;
    Int32                   rtnVal;

    GT_assert( GT_DEFAULT_MASK, taskArgs != NULL);

    pPSrvIoTask   =   (VPS_PSrvTaskObject *)taskArgs;
        ...
        while (TRUE)
        {
            ...
            if (pIoCmdStruct->cmdType == VPS_FVID2_CMDTYPE_COMPOSITE)
            {
                ...
                if (pIoCmdStruct->compositeCmdArgs != NULL)
                {
                    ...
                    for(counter = 0x0;
                        counter <= pIoCmdStruct->noOfCommands;
                        counter++)
                    {
            ...
            else if (pIoCmdStruct->cmdType == VPS_FVID2_CMDTYPE_SIMPLEX)
            {
                /*
            ...
            else
            {
                ...
                GT_2trace(PSTrace, GT_INFO,"From Processor %d, On notify no %d\n",
                                pIoCmdStruct->sysLnkProcId,
                                pIoCmdStruct->sysLnkNtyNo);
            }
        }
...
static Void     ntyCbCtrl(
                    ...
                    UInt32 payload)
{
    VPS_PSrvTaskObject      *pPSrvTaskHndl  = (VPS_PSrvTaskObject *)arg;
    VPS_PSrvCommandStruc    *pCmdStruct     = (VPS_PSrvCommandStruc *)payload;
    /* If the arg is NULL something is wrong in the system */
    GT_assert( GT_DEFAULT_MASK, arg != NULL);

    if (pCmdStruct != NULL)
    {
        /* Store the Processor ID, The control task would require
            this to make a choice to service this command or not */
    ...
    else
    {
        if (payload != NULL)
        {
            ((VPS_PSrvCommandStruc *) payload)->returnValue = VPS_EDRIVER_INUSE;
        }
    }
}
 ...
 *        VPS_PSrvCallback, which would have been populated during the stream
 *        creation.
 *
 * \param   handle    [IN]  Handle to the FVID2 stream, not used now.
 * \param   appData   [IN]  Pointer to structure of type VPS_PSrvCallback.
...
static Int32 completionCb(FVID2_Handle handle, Ptr appData, Ptr reserved)
{
    Int32            compCbHndl =   VPS_SOK;
#ifdef SYSLINKS_IPC_NOTIFY_INCLUDED
#ifndef VPS_PS_TASK_4_CB

    /*
     * appData will point to structure of type VPS_PSrvCallback. This structures
     ...
     */
    VPS_PSrvCallback *pPSrvCb   =   (VPS_PSrvCallback *)appData;
    ...
    compCbHndl  =   Notify_sendEvent(
                        pPSrvCb->sysLnkProcId,
                        VPS_PSRV_IPC_LINE_ID_USED,
                        pPSrvCb->sysLnkNtyNo,
                        (UInt32) appData,
                        TRUE);
#else /* VPS_PS_TASK_4_CB */
            ...
            UInt32 eventNo)
{
     ...
     */
    switch(((VPS_PSrvFvid2CmdOverlay *)simplexCommand)->command)
    {
        case VPS_FVID2_QUEUE:
        {
            VPS_PSrvFvid2QueueParams *qParams = (VPS_PSrvFvid2QueueParams *)
                                                    simplexCommand;
            GT_0trace(PSTrace, GT_INFO, "PS : FVID2 - Queuing up\n");
#ifdef ENABLE_AUTO_SC_FOR_YUV
		   /* Scalar Path is used to perform 420 to 422 Conversion */
            /* Before putting the frames in the display Q. We are passing
             * through the scalar path to convert 420 to 422 and as well
             * resize if required
             */
            scRtnValue = psScale(qParams);
            if (0 != scRtnValue)
            {
                Vps_printf("Scalar:Getting frames from scalar Failed\n\n");
                break;
            }
#endif
            /* Submit the resized/Chroma Upsampled (420 to 422 Conversion) to
             * the display Q for displaying*/
            qParams->returnValue =  FVID2_queue(
                                        qParams->fvid2Handle,
                                        qParams->frameList,
                                        qParams->streamId);
        ...
        case VPS_FVID2_DEQUEUE:
        {
            VPS_PSrvFvid2DequeueParams *dQParams =(VPS_PSrvFvid2DequeueParams *)
                                                        simplexCommand;
            GT_0trace(PSTrace, GT_INFO, "PS : FVID2 - De-Queuing\n");
            dQParams->returnValue =     FVID2_dequeue(
                                            dQParams->fvid2Handle,
                                            dQParams->frameList,
                                            dQParams->streamId,
                                            dQParams->timeout);
            fivd2RtnValue         =     dQParams->returnValue;
#ifdef ENABLE_AUTO_SC_FOR_YUV
            psScalarDequeue(dQParams);
#endif
            GT_1trace(PSTrace, GT_INFO, "PS : [Done] - rtn val %d\n",
                fivd2RtnValue);
            break;
        }
        case VPS_FVID2_PROCESS_FRAMES:
        {
            VPS_PSrvFvid2ProcessFramesParams *prsFrameParams = NULL;

            prsFrameParams = (VPS_PSrvFvid2ProcessFramesParams *)simplexCommand;

            GT_0trace(PSTrace, GT_INFO, "PS :FVID2-Queuing up to process\n");
            prsFrameParams->returnValue =   FVID2_processFrames(
                                                prsFrameParams->fvid2Handle,
        ...
        case VPS_FVID2_GET_PROCESSED_FRAMES:
        {
            VPS_PSrvFvid2GetProcessedFramesParams *gPrsFrameParams = NULL;

            GT_0trace(PSTrace,GT_INFO,"PS :FVID2-Getting processed frames\n");
            gPrsFrameParams =   (VPS_PSrvFvid2GetProcessedFramesParams *)
                                    simplexCommand;

            gPrsFrameParams->returnValue =  FVID2_getProcessedFrames(
                                                gPrsFrameParams->fvid2Handle,
        ...
        case VPS_FVID2_CONTROL:
        {
            FVID2_Format * setFmt;
            Int32 dataFormat;
            VPS_PSrvFvid2ControlParams *ctrlParams = NULL;

            GT_0trace(PSTrace,GT_INFO,"PS : FVID2 - Control\n");
            ctrlParams = (VPS_PSrvFvid2ControlParams *) simplexCommand;
            setFmt = (FVID2_Format *)(ctrlParams->cmdArgs);
            if(ctrlParams->cmd == FVID2_SET_FORMAT )
            {
                dataFormat = setFmt->dataFormat;
            }
#ifdef ENABLE_AUTO_SC_FOR_YUV
            psScalarControl(ctrlParams);
#endif
            if(ctrlParams->cmd == IOCTL_VPS_DEI_DISP_SET_PARAMS)
            {
               ctrlParams->returnValue = VPS_SOK;
               fivd2RtnValue = ctrlParams->returnValue;
               break;
            }
            ctrlParams->returnValue =   FVID2_control(
                                            ctrlParams->fvid2Handle,
                                            ctrlParams->cmd,
                                            ctrlParams->cmdArgs,
        ...
        case VPS_FVID2_DELETE:
        {
             ...
             */
            VPS_PSrvFvid2DeleteParams   *delParams      =
                (VPS_PSrvFvid2DeleteParams *) simplexCommand;

            fivd2RtnValue  =   VPS_EBADARGS;
...
#ifdef ENABLE_AUTO_SC_FOR_YUV
            fivd2RtnValue  = psScalarDelete(delParams);
#endif
            fivd2RtnValue  =    FVID2_delete(
                                    delParams->fvid2Handle,
                                    delParams->deleteArgs);
            if (fivd2RtnValue == VPS_SOK)
            {
        ...
        default:
        {
            ...
            /* Let the remote agent / client know that the command is not
               supported */
            ((VPS_PSrvFvid2CmdOverlay *)simplexCommand)->returnValue =
                                                           VPS_EUNSUPPORTED_CMD;
        }
    }
 ...
 * \param   None
 *
 * \return  VPS_EALLOC if no notification numbers are available other wise a +
 *          number.
 */

static Int32 allocSysLnkNtyNumber(UInt32 sysLnkProcId)
{
    Int32                   returnValue =   VPS_EALLOC;
    UInt32                  procIdIndex, ntyNoIndex;
    VPS_PSrvNtyNumObject    *pProcNtyNoInst;

    GT_assert( GT_DEFAULT_MASK, sysLnkProcId < VPS_PSRV_MAX_NO_OF_CORES);
    ...
    GT_1trace(PSTrace,GT_INFO,"PS : no for processor %d\n",
        sysLnkProcId);
    for(procIdIndex = 0x0;
        procIdIndex < VPS_PSRV_MAX_NO_OF_CORES;
        procIdIndex++)
    {
        if (gPSrvNtyNo4AllCores[procIdIndex].determinedProcId == sysLnkProcId)
        {
                ...
                ntyNoIndex++)
            {
                if ((pProcNtyNoInst[ntyNoIndex].status)
                    == VPS_PSRV_NOTIFY_FREE)
                {
                    pProcNtyNoInst[ntyNoIndex].status
                        = VPS_PSRV_NOTIFY_OCCUPIED;
                ...
                ntyNoIndex++)
            {
                if ((pProcNtyNoInst[ntyNoIndex].ntyNumber) == sysLnkNtyNo)
                {
                            ...
                            VPS_PSRV_NOTIFY_FREE)
                    {
                        pProcNtyNoInst[ntyNoIndex].status
                            = VPS_PSRV_NOTIFY_FREE;
                        GT_1trace(PSTrace,GT_INFO,"PS : DeAllocated notify %d\n",
                        pProcNtyNoInst[ntyNoIndex].ntyNumber);
                    }
                    else
                    {
                        GT_1trace(PSTrace,GT_INFO,"PS : %d-is already released\n",
                        pProcNtyNoInst[ntyNoIndex].ntyNumber);
                    }
                    break;
                }
               ...
               Uint32  remoteProcId)
{
    ...
    Int32                   allocatedNtyNo  =   VPS_EALLOC;
    ...
    while(TRUE)
    {
        ...
        if ((creatParam->hostTaskInstance <= VPS_FVID2_TASK_TYPE_LOWER_GUARD)  &&
            (creatParam->hostTaskInstance >= VPS_FVID2_TASK_TYPE_UPPER_GUARD))
        {
            /* Wrong stream type */
            creatParam->fvid2Handle = NULL;
            *((Int32 *)(creatParam->createStatusArgs)) = VPS_EUNSUPPORTED_OPS;
            GT_0trace(PSTrace, GT_ERR, "PS : Error - wrong stream type\n");
            break;
        }

        /* Check for the pointer required by Proxy Server, other params will be
           be validated by FVID2 API call. Rely on it */
        ...
        if ((allocatedNtyNo > VPS_PSRV_MAX_NO_NOTIFY) ||
            (allocatedNtyNo < 0x0u))
        {
            /* Out of notification numbers */
            creatParam->fvid2Handle = NULL;
            if (creatParam->ioReqCb != NULL)
            {
        ...
        /* Step C. Associate notify with right class of stream
                   handling task */
#ifdef SYSLINKS_IPC_NOTIFY_INCLUDED
        if (Notify_registerEvent(
                ...
                ntyCbGeneric,
                (UArg)
                &(gPSrvTaskObjInstances[creatParam->hostTaskInstance])) !=
                Notify_S_SUCCESS )
        {
            deallocSysLnkNtyNumber(remoteProcId, allocatedNtyNo);
            *((Int32 *)(creatParam->createStatusArgs))  = VPS_EALLOC;
        ...
        if (callbacksGiven == TRUE)
        {
            ...
            creatParam->ioReqCb->cbType         = VPS_FVID2_IO_CALLBACK;

            creatParam->errCb->appCallBack      =
                                                creatParam->cbParams->errCbFxn;
            creatParam->errCb->errList          = creatParam->cbParams->errList;
            creatParam->errCb->sysLnkProcId     = remoteProcId;
            creatParam->errCb->cbType           = VPS_FVID2_ERR_CALLBACK;
 ...
 *
 * \return  VPS_SOK if successful, error code otherwise.
 */

Int32 initSysLnkNtyShm(VPS_PSrvInitParams *pSysLnkNtyInitParams)
{
    /*
     *  For each core
    ...
    for(procIdIndex = 0x0;
        procIdIndex < pSysLnkNtyInitParams->sysLnkNoOfCores;
        procIdIndex++)
    {
        /* Step 0 and 1 */
 ...
 * \return  VPS_SOK if successful, error code otherwise.
 */
static Int32 initializeClientNtfyTask(VPS_PSrvInitParams *pPsNtfyInitParams)
{
    Int32               rtnValue        =   VPS_SOK;
    VPS_PSrvTaskObject  *pPSrvNtfyTsk   =   NULL;
    UInt32              counter;
    Semaphore_Params    semParams;
    Task_Params         taskParams;
#ifdef VPS_PS_TASK_4_CB
    while (TRUE)
    {
        ...
        counter = VPS_PSRV_NTFY_TASK_MAX_Q_DEPTH;

        Semaphore_Params_init(&semParams);
        pPSrvNtfyTsk->tSem = Semaphore_create(
                                    VPS_PSRV_NTFY_TASK_MAX_Q_DEPTH,
                                    &semParams,
                                    NULL);
        ...
        */
       rtnValue =  VpsUtils_queCreate(
                           &(pPSrvNtfyTsk->spaceHolder4QHnld),
                           VPS_PSRV_NTFY_TASK_MAX_Q_DEPTH,
        ...
        {
            rtnValue    =   VPS_EALLOC;
            GT_0trace(PSTrace, GT_ERR, "PS : Error - Could not create Q for \n");
            GT_0trace(PSTrace, GT_ERR, "Completion task\n");
            break;
        }
        pPSrvNtfyTsk->cmdQHndl  =
            &(pPSrvNtfyTsk->spaceHolder4QHnld);
        ...
        pPSrvNtfyTsk->taskInstance  = VPS_FVID2_TASK_TYPE_UPPER_GUARD;

        pPSrvNtfyTsk->taskOwnHndl   =   Task_create(
                                                psNtfyClientTask,
                                                &taskParams,
                                                NULL);
        break;
...
static Void psNtfyClientTask(UInt32 taskArgs, UInt32 taskArgsUnused)
{
    ...
    while (TRUE)
    {
        ...
        while (TRUE)
        {
            ...
            {
                pPSrvCb = (VPS_PSrvCallback *)tempVar;
 ...
 *
 * 1. Covert all absolute address passed by remote agent to M3 VPSS virtual
 *    memory.
