...
Int32 SwMsLink_getLayoutInfo(SwMsLink_LayoutParams *layoutParams, SwMsLink_LayoutInfo *layoutInfo, UInt32 outPitch)
{
    ...
    switch(layoutParams->outLayoutMode)
    {
        ...
        case SYSTEM_LAYOUT_MODE_4CH:
            ...
            for(row=0; row<2; row++)
            {
                for(col=0; col<2; col++)
                {
                    ...
                    winInfo->height = VpsUtils_align(outHeight/2, heightAlign);
        ...
        case SYSTEM_LAYOUT_MODE_8CH:
            ...
            for(row=0; row<2; row++)
            {
                for(col=0; col<2; col++)
                {
                    ...
                    winInfo->width  = VpsUtils_align((outWidth*2)/5, widthAlign);
                    winInfo->height = VpsUtils_align(outHeight/2, heightAlign);
                    winInfo->startX = winInfo->width*col;
                    winInfo->startY = winInfo->height*row;
                    winInfo->bypass = FALSE;
        ...
        case SYSTEM_LAYOUT_MODE_5CH_PLUS_1CH:
            ...
            for(row=0; row<2; row++)
            {
                ...
                winInfo->width  = VpsUtils_align(layoutInfo->winInfo[0].width/2, widthAlign);
                winInfo->height = VpsUtils_align(layoutInfo->winInfo[0].height/2, heightAlign);
        ...
        case SYSTEM_LAYOUT_MODE_7CH_PLUS_1CH:
            ...
            for (row = 0; row < 3; row++)
            {
                ...
                winInfo->width  = VpsUtils_align(
                                    outWidth - layoutInfo->winInfo[0].width,
                                    widthAlign);
                winInfo->height = VpsUtils_align(
        ...
        case SYSTEM_LAYOUT_MODE_1CH_PLUS_2CH_PIP:
            ...
            for(col=0; col<2; col++)
            {
                ...
                {
                    winInfo->startX = VpsUtils_align(
                        outWidth - winInfo->width - outWidth/20,
                        widthAlign);
                }

                winInfo->startY = VpsUtils_align(
    ...
    /* if a channel repeats in different window, then mark the repeated window
        as having invalid CH
    */
    memset(layoutInfo->ch2WinMap, SYSTEM_SW_MS_INVALID_ID, sizeof(layoutInfo->ch2WinMap));
    for(winId=0; winId<layoutInfo->numWin; winId++)
    {
        ...
        {
            if(layoutInfo->ch2WinMap[winInfo->channelNum]==SYSTEM_SW_MS_INVALID_ID)
