...
static Int32 DispHwMsLink_createLayouts(DispHwMsLink_Obj *pObj)
{
    ...
    /* Set the window position and size for all the windows.
     * Start from the first input Queue, then second and so on... */
    index = 0u;
    for (qIndex = 0u; qIndex < pObj->createPrms.numInQue; qIndex++)
    {
        for (winCnt = 0u; winCnt < pObj->inQueInfo[qIndex].numCh; winCnt++)
        {
            ...
            /* Get the window dimension from previous link channel info.
             * It is assumed that this and position are done properly
             * by system configuration table. */
            ...
            Vps_rprintf(" %d: DISPLAY: WIN%d: (%d , %d) (%d x %d) %dB\n", Clock_getTicks(),
                index,
                winFmt->winStartX, winFmt->winStartY,
                winFmt->winWidth, winFmt->winHeight,
                winFmt->pitch[0u]
            );
            #endif
        }
                             ...
                             DispHwMsLink_CreateParams *pPrm)
{
    ...
    GT_assert( GT_DEFAULT_MASK, pObj->createPrms.numInQue <= DISP_HWMS_LINK_MAX_IN_QUE);
    ...
    for (qIndex = 0u; qIndex < pObj->createPrms.numInQue; qIndex++)
    {
        ...
        GT_assert( GT_DEFAULT_MASK, pInTskInfo != NULL);
        GT_assert( GT_DEFAULT_MASK, pPrm->inQueParams[qIndex].prevLinkQueId < pInTskInfo->numQue);
        memcpy(
            &pObj->inQueInfo[qIndex],
...
Int32 DispHwMsLink_drvPrintRtStatus(DispHwMsLink_Obj *pObj, UInt32 elaspedTime)
{
    ...
    Vps_rprintf(" %d: DISPLAY: %s: %d fps, Callback Interval: Max = %d, Min = %d !!! \r\n",
        Clock_getTicks(),
        displayName[pObj->dispInstId],
...
static Int32 DispHwMsLink_drvDqFrames(DispHwMsLink_Obj *pObj)
{
    ...
    for (qIndex = 0u; qIndex < DISP_HWMS_LINK_MAX_IN_QUE; qIndex++)
    {
    ...
    for (dispFrmCnt = 0u; dispFrmCnt < dispFrmList.numFrames; dispFrmCnt++)
    {
        ...
        if (DISP_HWMS_LINK_BLK_FRM_CH_NUM != pFrm->channelNum)
        {
            ...
            GT_assert( GT_DEFAULT_MASK, pFrameInfo->displayHwMsRepeatCount != 0u);
            pFrameInfo->displayHwMsRepeatCount--;
            if (0u == pFrameInfo->displayHwMsRepeatCount)
            {
                /* Frame is not repeated, so give the frame to previous
                 * link. Before that change back the channel number to the
                 * original one as needed by each input links. */
                tempCnt = 0u;
                effQIndex = 0u;
                for (qIndex = 0u; qIndex < pObj->createPrms.numInQue; qIndex++)
                {
                    /* Figure out the input queue to which this frame
                     * belongs */
                    if (pFrm->channelNum <
    ...
    for (qIndex = 0u; qIndex < pObj->createPrms.numInQue; qIndex++)
    {
        ...
        if (inFrmList.numFrames > 0u)
        {
            ...
            for (frmCnt = 0u; frmCnt < inFrmList.numFrames; frmCnt++)
            {
                ...
                /* Assign channel number based on display needs and queue
                 * it to the respective queue */
                GT_assert( GT_DEFAULT_MASK, pFrm->channelNum < pObj->inQueInfo[qIndex].numCh);
