...
Int32 DisplayLink_tskRun(DisplayLink_Obj *pObj, VpsUtils_TskHndl *pTsk, VpsUtils_MsgHndl **pMsg, Bool *done, Bool *ackMsg)
{
    ...
    while(!runDone)
    {
        ...
        switch(cmd)
        {
            ...
            case DISPLAY_LINK_CMD_SWITCH_CH:
                status = DisplayLink_drvSwitchCh(pObj, (UInt32)VpsUtils_msgGetPrm(pRunMsg));
                VpsUtils_tskAckOrFreeMsg(pRunMsg, status);
                break;
            default:
                VpsUtils_tskAckOrFreeMsg(pRunMsg, status);
...
Void DisplayLink_tskMain(struct VpsUtils_TskHndl *pTsk, VpsUtils_MsgHndl *pMsg)
{
    ...
    while(!done)
    {
        ...
        switch(cmd)
        {
            case SYSTEM_CMD_START:
                ...
                if(status==FVID2_SOK)
                {
                    status = DisplayLink_tskRun(pObj, pTsk, &pMsg, &done, &ackMsg);
                }
            ...
            case SYSTEM_CMD_DELETE:
                ...
                break;
            case DISPLAY_LINK_CMD_SWITCH_CH:
                status = DisplayLink_drvSwitchCh(pObj, (UInt32)VpsUtils_msgGetPrm(pMsg));
                VpsUtils_tskAckOrFreeMsg(pMsg, status);
                break;
            default:
                VpsUtils_tskAckOrFreeMsg(pMsg, status);
