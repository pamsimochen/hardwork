...
Int32 DupLink_drvCreate(DupLink_Obj *pObj, DupLink_CreateParams *pPrm)
{
    ...
    GT_assert( GT_DEFAULT_MASK, pObj->createArgs.numOutQue<DUP_LINK_MAX_OUT_QUE);
    ...
    for(outId=0; outId<DUP_LINK_MAX_OUT_QUE; outId++)
    {
        ...
        {
            pFrame = &pObj->frames[DUP_LINK_MAX_FRAMES_PER_OUT_QUE*outId + frameId];
            pFrameInfo = &pObj->frameInfo[DUP_LINK_MAX_FRAMES_PER_OUT_QUE*outId + frameId];

            pFrame->appData = pFrameInfo;

            status = VpsUtils_bufPutEmptyFrame(&pObj->outFrameQue[outId], pFrame);
            GT_assert(GT_DEFAULT_MASK, status==FVID2_SOK);
        }
    }
...
Int32 DupLink_getFullFrames(VpsUtils_TskHndl *pTsk, UInt16 queId, FVID2_FrameList *pFrameList)
{
    DupLink_Obj *pObj = (DupLink_Obj *)pTsk->appData;
    ...
    return VpsUtils_bufGetFull(&pObj->outFrameQue[queId], pFrameList, BIOS_NO_WAIT);
}
    ...
    if(pObj->inFrameList.numFrames)
    {
        ...
        for(frameId=0; frameId<pObj->inFrameList.numFrames; frameId++)
        {
            ...
            for(outId=0; outId<pCreateArgs->numOutQue; outId++)
            {
                ...
                pObj->outFrameList[outId].frames[
                    pObj->outFrameList[outId].numFrames] = pFrame;
        ...
        for(outId=0; outId<pCreateArgs->numOutQue; outId++)
        {
            status = VpsUtils_bufPutFull(&pObj->outFrameQue[outId], &pObj->outFrameList[outId]);
            GT_assert(GT_DEFAULT_MASK, status==FVID2_SOK);

            if(pCreateArgs->notifyNextLink)
            {
                System_sendLinkCmd(
                    pCreateArgs->outQueParams[outId].nextLink,
                    SYSTEM_CMD_NEW_DATA
                );
            }
        }
...
Int32 DupLink_putEmptyFrames(VpsUtils_TskHndl *pTsk, UInt16 queId, FVID2_FrameList *pFrameList)
{
    DupLink_Obj *pObj = (DupLink_Obj *)pTsk->appData;
