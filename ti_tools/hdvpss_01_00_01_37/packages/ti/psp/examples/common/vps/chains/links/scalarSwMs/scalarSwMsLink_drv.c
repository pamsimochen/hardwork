...
Int32 ScalarSwMsLink_drvFvidErrCb(
                FVID2_Handle handle,
                Ptr appData,
                Ptr errList,
                Ptr reserved)
...
Int32 ScalarSwMsLink_drvSwitchLayout(ScalarSwMsLink_Obj *pObj, UInt32 layoutId, UInt32 startChannelNum)
{
    ...
    switch(pObj->layoutId)
    {
        ...
        case SYSTEM_LAYOUT_MODE_4CH:
            ...
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth *1;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + pWinObj->outHeight*0;

            idx++;
            pObj->winObj[idx].outWidth  = pWinObj->outWidth;
            pObj->winObj[idx].outHeight = pWinObj->outHeight;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth *0;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + pWinObj->outHeight*1;
        ...
        case SYSTEM_LAYOUT_MODE_8CH:
            ...
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth*2;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + (pWinObj->outHeight/2)*0;

            idx++;
            pObj->winObj[idx].outWidth  = outWidth - pWinObj->outWidth*2;
            pObj->winObj[idx].outHeight = pWinObj->outHeight/2;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth*2;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + (pWinObj->outHeight/2)*1;

            idx++;
            pObj->winObj[idx].outWidth  = outWidth - pWinObj->outWidth*2;
            pObj->winObj[idx].outHeight = pWinObj->outHeight/2;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth*2;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + (pWinObj->outHeight/2)*2;

            idx++;
            pObj->winObj[idx].outWidth  = outWidth - pWinObj->outWidth*2;
            pObj->winObj[idx].outHeight = pWinObj->outHeight/2;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth*2;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + (pWinObj->outHeight/2)*3;
            break;
        ...
        case SYSTEM_LAYOUT_MODE_16CH:
            ...
            pWinObj->outStartX = VpsUtils_floor(pWinObj->outStartX, VPS_BUFFER_ALIGNMENT);
            pWinObj->outWidth  = VpsUtils_floor(pWinObj->outWidth , VPS_BUFFER_ALIGNMENT);
            ...
            pObj->winObj[idx].outWidth  = pWinObj->outWidth;
            pObj->winObj[idx].outHeight = pWinObj->outHeight;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth *3;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + pWinObj->outHeight*0;
            ...
            pObj->winObj[idx].outHeight = pWinObj->outHeight;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth *0;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + pWinObj->outHeight*2;
            ...
            pObj->winObj[idx].outHeight = pWinObj->outHeight;
            pObj->winObj[idx].outStartX = pWinObj->outStartX + pWinObj->outWidth *0;
            pObj->winObj[idx].outStartY = pWinObj->outStartY + pWinObj->outHeight*3;
    ...
    for(winId=0; winId<pObj->numWin; winId++)
    {
        ...
        pWinObj->outBufAddrOffset[0] =
                pWinObj->outStartY * pOutInfo->pitch[0]
              + pWinObj->outStartX * 2;
...
Int32 ScalarSwMsLink_drvCreateOutInfo(ScalarSwMsLink_Obj *pObj, UInt32 outRes)
{
    ...
    status = VpsUtils_memFrameAlloc(&pObj->outFrameFormat, pObj->outFrames, SCALAR_SW_MS_LINK_MAX_OUT_FRAMES);
    GT_assert( GT_DEFAULT_MASK, status==FVID2_SOK);

    for(frameId=0; frameId<SCALAR_SW_MS_LINK_MAX_OUT_FRAMES; frameId++)
    {
        status = VpsUtils_bufPutEmptyFrame(&pObj->bufOutQue, &pObj->outFrames[frameId]);
        GT_assert( GT_DEFAULT_MASK, status==FVID2_SOK);
        ...
        Vps_rprintf(" %d: SW_SWMS: %2d: 0x%08x, %d x %d, %08x B\n",
            ...
            pObj->outFrameFormat.pitch[0]/2, pObj->outFrameFormat.height, pObj->outFrameFormat.height*pObj->outFrameFormat.pitch[0]);
...
Int32 ScalarSwMsLink_drvCreateWinObj(ScalarSwMsLink_Obj *pObj, UInt32 winId)
{
    ...
    &&   FVID2_BUF_FMT_FIELD == pChInfo->bufferFmt))
    ...
    for (fid = 0; fid < FVID2_MAX_FIELDS; fid++)
    {
        ...
        if(pObj->layoutId==SYSTEM_LAYOUT_MODE_4CH && winId < pObj->inQueInfo.numCh)
        {
        ...
        if (winId == 0 && fid == 0)
        {
            if (VPS_VPDMA_MT_TILEDMEM == pChInfo->memType)
            {
                status = VpsUtils_tilerFrameAlloc(
                            pFormat, &pWinObj->blankFrame[fid], 1);
            }
            else
            {
...
Int32 ScalarSwMsLink_drvCreate(ScalarSwMsLink_Obj *pObj, ScalarSwMsLink_CreateParams *pPrm)
{
    ...
    pObj->scCreateParams.numChannels = pObj->numWin*FVID2_MAX_FIELDS;
    ...
    switch(pObj->linkId)
    {
        ...
        case SYSTEM_LINK_ID_SCALAR_SW_MS_3:
            ...
            break;
        case SYSTEM_LINK_ID_SCALAR_SW_MS_4:
...
Int32 ScalarSwMsLink_drvProcessData(ScalarSwMsLink_Obj *pObj)
{
    ...
    if(frameList.numFrames)
    {
        ...
        for(frameId=0; frameId < frameList.numFrames; frameId++)
        {
            ...
            {
                status = VpsUtils_bufPutFullFrame(&pObj->winObj[winId].bufInQue[pFrame->fid], pFrame);
                if(status!=FVID2_SOK)
                {
...
Int32 ScalarSwMsLink_drvMakeFrameLists(
                             ...
                             )
{
    ...
    for(winId=0; winId<SCALAR_SW_MS_LINK_MAX_WIN; winId++)
    {
        ...
        if(winId<pObj->numWin)
        {
            for(fid=0; fid<FVID2_MAX_FIELDS; fid++)
            {
                VpsUtils_bufGetFullFrame(&pWinObj->bufInQue[fid], &pInFrame[fid], BIOS_NO_WAIT);

                if(!pWinObj->isInterlaced && fid > 0)
                {
                    if(pInFrame[fid])
                    {
                        freeFrameList->frames[freeFrameList->numFrames] = pInFrame[fid];
                ...
                if(pInFrame[fid])
                {
                    ...
                    {
                        freeFrameList->frames[freeFrameList->numFrames] = pWinObj->pCurInFrame[fid];
                        freeFrameList->numFrames++;
                    }
                ...
                if(pWinObj->pCurInFrame[fid])
                {
                    ...
                    inFrameList->frames[inFrameList->numFrames] = pWinObj->pCurInFrame[fid];
                }
                else
                {
                    ...
                    inFrameList->frames[inFrameList->numFrames] = &pWinObj->blankFrame[fid];
                }
                inFrameList->numFrames++;

                outFrameList->frames[outFrameList->numFrames] = &pWinObj->curOutFrame[fid];
...
Int32 ScalarSwMsLink_drvDoScaling(ScalarSwMsLink_Obj *pObj)
{
    ...
    status = VpsUtils_bufGetEmptyFrame(&pObj->bufOutQue, &pOutFrame, BIOS_NO_WAIT);
    if(status!=FVID2_SOK)
        return status;

    ScalarSwMsLink_drvLock(pObj);

    ScalarSwMsLink_drvMakeFrameLists(pObj, &pObj->inFrameList, &pObj->outFrameList, &pObj->freeFrameList, pOutFrame);
    ...
    if(pObj->inFrameList.numFrames)
    {
        ...
        if(pObj->vipInstId!=SYSTEM_VIP_MAX)
        {
            ...
            {
                // VIP was reset since last frame processing, so we need to reload VIP-SC co-effs
                ScalarSwMsLink_drvSetScCoeffs(pObj);
            }
        }
        ...
        status = FVID2_getProcessedFrames(pObj->fvidHandle, &pObj->processList, BIOS_NO_WAIT);
        GT_assert( GT_DEFAULT_MASK, status==FVID2_SOK);
...
Int32 ScalarSwMsLink_drvDelete(ScalarSwMsLink_Obj *pObj)
{
    ...
    VpsUtils_memFrameFree(&pObj->outFrameFormat, pObj->outFrames, SCALAR_SW_MS_LINK_MAX_OUT_FRAMES);
    ...
    {
        channelNum = ScalarSwMsLink_makeChannelNum(0, 0);
        pScParams  = &pObj->winScParams[channelNum];
        pFormat    = &pScParams->inFmt;
        VpsUtils_memFrameFree(pFormat, &pObj->winObj[0].blankFrame[0], 1);
    }
