...
Int32 SwMsLink_drvDeiBypassFvidCb(FVID2_Handle handle, Ptr appData, Ptr reserved)
{
    SwMsLink_Obj *pObj = (SwMsLink_Obj*)appData;
...
(
    SwMsLink_Obj          *pObj,
    SwMsLink_LayoutParams *layoutParams
)
{
    UInt32              winId, chNum;
    ...
    for (winId = 0; winId < pObj->layoutInfo.numWin; winId++)
    {
        ...
        pWinObj->scRtOutFrmPrm.memType  = pObj->info.queInfo[0].chInfo[0].memType;
        ...
        else
        {
            ...
            /* In case captured data is progressive and window is bypass (don't
               care quality so much), SC takes only even lines and make S/W
               mosaic. This is sometimes needed due to SC performance. */
                ...
                pObj->layoutInfo.winInfo[winId].bypass)
            {
                ...
                pWinObj->scRtInFrmPrm.dataFormat  =
                    pObj->inQueInfo.chInfo[chNum].bufferFmt;
            }
            else
            {
...
Int32 SwMsLink_drvCreateOutInfo(SwMsLink_Obj *pObj, UInt32 outRes)
{
    ...
    /* alloc buffer of max possible size but use only what is needed for a given resolution */
    status = VpsUtils_memFrameAlloc(
                &pObj->bufferFrameFormat,
                pObj->outFrames,
                SW_MS_LINK_MAX_OUT_FRAMES);
    GT_assert(GT_DEFAULT_MASK, status == FVID2_SOK);
    ...
    {
        status = VpsUtils_bufPutEmptyFrame(
                    &pObj->bufOutQue,
...
Int32 SwMsLink_drvCreateScDrv(SwMsLink_Obj *pObj)
{
    ...
    pDrvObj->fvidHandle  = FVID2_create(
                                ...
                                &pDrvObj->scCreateStatus,
                                &cbParams);
    GT_assert(GT_DEFAULT_MASK, pDrvObj->fvidHandle != NULL);

    SwMsLink_drvSetScCoeffs(pDrvObj->fvidHandle, FALSE, FALSE);    /* only downscale for SDTV */
...
Int32 SwMsLink_drvProcessData(SwMsLink_Obj *pObj)
{
    ...
    if (frameList.numFrames)
    {
        ...
        for (frameId = 0; frameId < frameList.numFrames; frameId++)
        {
            ...
            {
                /* win is not assigned to this ch, normally this condition wont happen */
                frameList.frames[freeFrameNum] = pFrame;
                freeFrameNum++;
                continue;
            }
            ...
            /* valid win and ch */
            /* [TODO] For the moment, SC drv takes only even fields and make S/W
               moasic. It throw away odd fields. Later, it can take both fields
               and make S/W mosaic in merged frame format. */
            if (pWinInfo->bypass || !pObj->isDei)
            {
                /* window shows channel in bypass mode,
                    then drop odd fields, i.e always expect even fields */
                pWinObj->expectedFid = 0;
            }
...
Int32 SwMsLink_drvMakeFrameLists(SwMsLink_Obj *pObj, FVID2_Frame *pOutFrame)
{
    ...
    for (winId = 0; winId < SYSTEM_SW_MS_MAX_WIN; winId++)
    {
        ...
        {
            /* empty input queue */
            status = VpsUtils_bufGetFullFrame(
                        &pWinObj->bufInQue,
                        &pInFrame,
                        BIOS_NO_WAIT);
            if (status == FVID2_SOK)
        ...
        if (pObj->isDei)
        {
            if (pWinInfo->bypass)
            {
                ...
                /* [TODO] This is for avoiding bypass DEI driver getting
                   FID = 1. Not clear why FID = 1 is passed to bypass DEI
...
Int32 SwMsLink_deiDrvProcessFrames(SwMsLink_Obj *pObj, SwMsLink_DeiDrvObj *pDrvObj)
{
    ...
    if (pDrvObj->inFrameList.numFrames)
    {
        ...
        #ifdef SYSTEM_DEBUG_SWMS_RT
        Vps_printf(
            " %d: SWMS: Processing of %d frames done !!!\n",
            Clock_getTicks(),
            pDrvObj->inFrameList.numFrames
        );
        #endif
...
Int32 SwMsLink_scDrvProcessFrames(SwMsLink_Obj *pObj, SwMsLink_ScDrvObj *pDrvObj)
{
    Int32 status = FVID2_SOK;
    UInt32 curTime;
...
Int32 SwMsLink_drvDelete(SwMsLink_Obj *pObj)
{
    ...
    {
        SwMsLink_drvDeleteDrv(
            pObj,
            FALSE   // don't care for SC driver
        );
    }
    ...
    VpsUtils_memFrameFree(
        &pObj->bufferFrameFormat,
        pObj->outFrames,
        SW_MS_LINK_MAX_OUT_FRAMES);
    VpsUtils_memFrameFree(&pObj->bufferFrameFormat, &pObj->winObj[0].blankFrame, 1);
