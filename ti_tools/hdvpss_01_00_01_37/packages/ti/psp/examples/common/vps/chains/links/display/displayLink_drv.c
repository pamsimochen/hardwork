...
Int32 DisplayLink_drvDisplayAllocAndQueBlankFrame(DisplayLink_Obj *pObj)
{
    ...
    for(numQue=0; numQue<DISPLAY_LINK_BLANK_FRAME_INIT_QUE; numQue++)
    {
        ...
        pObj->blankFrame.addr[1][0] = (UInt8*)pObj->blankFrame.addr[0][0] + pObj->displayFormat.pitch[0];
...
Int32 DisplayLink_drvDisplayCreate(DisplayLink_Obj *pObj)
{
    ...
    {
        pFormat->scanFormat = FVID2_SF_INTERLACED;
        if (TRUE == pObj->createArgs.fieldsMerged)
        {
    ...
    Vps_printf(" %d: DISPLAY: %d: Window size %dx%d, %dB",
        Clock_getTicks(),
        pObj->displayInstId, pFormat->width, pFormat->height, pFormat->pitch[0]
    );
    #endif

    pObj->displayHndl = FVID2_create(
                            FVID2_VPS_DISP_DRV,
                            pObj->displayInstId,
                            &pObj->displayCreateArgs,
                            &pObj->displayCreateStatus,
                            &cbParams
                        );
...
Int32 DisplayLink_drvProcessData(DisplayLink_Obj *pObj)
{
    ...
    do
    {
        ...
        for(displayFrameId=0; displayFrameId<displayFrameList.numFrames; displayFrameId++)
        {
        ...
        if(displayFrameList.numFrames)
        {
            ...
            Vps_printf(" %d: DISPLAY: Deque %d frames\n", Clock_getTicks(), displayFrameList.numFrames);
            #endif
        }
    ...
    if(frameList.numFrames)
    {
        ...
        for(frameId=0; frameId<frameList.numFrames; frameId++)
        {
            ...
            if(pFrame->channelNum==pObj->curDisplayChannelNum)
            {
                ...
                pFrame->addr[1][0] = (UInt8*)pFrame->addr[0][0] + pObj->displayFormat.pitch[0];
