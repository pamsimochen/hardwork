...
Void Chains_swMsTimerCb(UArg arg)
{
    ...
    if(gChains_swMsCtrl.timerCount%CHAINS_SW_MS_AUTO_LAYOUT_SWITCH_INTERVAL_IN_SECS==0)
    {
        gChains_swMsCtrl.autoSwitchLayout = TRUE;
    }
    if(gChains_swMsCtrl.timerCount%CHAINS_SW_MS_AUTO_CH_SWITCH_INTERVAL_IN_SECS==0)
    {
...
Void Chains_swMsGetLayoutPrm(UInt32 displayNum, Bool switchLayout, Bool switchCh, SwMsLink_LayoutParams *pLayoutPrm)
{
    UInt32 numLayouts;
    UInt32 numCh;
    UInt32 winId, chId;
    ...
    numLayouts = sizeof(gChains_swMsLayoutList)/sizeof(gChains_swMsLayoutList[0]);
    ...
    pLayoutPrm->outLayoutMode =gChains_swMsLayoutList[ gChains_swMsCtrl.layoutId[displayNum] ];
            ...
            UInt32 numDisplay)
{
    ...
    {
       Chains_swMsGetLayoutPrm(i, switchLayout, switchCh, &swMsPrm[i].layoutPrm);
       System_linkControl(swMsLinkId[i], SYSTEM_SW_MS_LINK_CMD_SWITCH_LAYOUT, &swMsPrm[i].layoutPrm, TRUE);
    }
