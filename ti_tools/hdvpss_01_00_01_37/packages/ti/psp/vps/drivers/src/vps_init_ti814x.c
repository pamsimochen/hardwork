...
/**
 * \file vps_init_ti814x.c
 *
 * \brief Vps initialization file for TI814X platform
...
#define SD_DAC_CTRL_REG_ADDR            (CSL_TI814x_CTRL_MODULE_BASE + \
                                            0x0670)
...
VpsHal_Handle HdVencHandles[CSL_VPS_HD_VENC_PER_CNT] = {NULL, NULL};
VpsHal_Handle GrpxHandles[CSL_VPS_GRPX_PER_CNT] = {NULL, NULL, NULL};

CSL_VpsRegsOvly  gVpsHal_regsVps   = (CSL_VpsRegsOvly)CSL_TI814x_VPS_BASE;
...
static Int32 vpsiDCtrlInit(void)
{
    ...
    if (0 == retVal)
    {
        ...
        dcInitParams.outNode[2u].vencHalHandle = SdVencHandle;
...
static Int32 vpsiHalOpen(void)
{
    ...
    if (0 == retVal)
    {
        ...
        * (volatile UInt32 *)(SD_DAC_CTRL_REG_ADDR) = 0x18800DB;
#endif
    ...
    if (0 == retVal)
    {
        HdVencHandles[1u] = VpsHal_hdvencOpen(VPSHAL_HDVENC_DVO2);
