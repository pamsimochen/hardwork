...
static Int32 Vps_i2cTransferMsgPoll(PSP_I2cDriverObject *i2cDrvObj,
                        PSP_I2cXferParams *i2cXferParams);
static Int32 Vps_i2cTransferMsgInt(PSP_I2cDriverObject *i2cDrvObj,
                        PSP_I2cXferParams *i2cXferParams);
...
UInt32 numOfSemPends = 0;
UInt32 numOfSemPosts = 0;
UInt32 numOfSemClr = 0;
UInt32 isIsrValid = 0;
UInt8 semindex = 0;
UInt32 semOrder[300] = {0};
UInt32 semTimeOrder[300] = {0};
UInt32 isrStatus[3][300] = {0};
UInt32 isrStatusloop[300] = {0};
UInt8 isrindexloop = 0;
UInt8 isrIndex = 0;
UInt8  index = 0;
UInt32 isrstat[300] = {0};
                ...
                PSP_I2cInitParams *initParams)
{
    ...
    /* Validate arguments */
    if ((numInstance > VPS_DEVICE_I2C_INST_ID_MAX)
        || (NULL == initParams))
    {
    ...
    else
    {
        ...
        if ((FVID2_SOK == retVal) &&
            (initParams->opMode == I2C_OPMODE_DMA))
        {
            retVal = FVID2_EBADARGS;
        }
    }
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    Vps_printf ( " I2C: completionSem semaphore create failed !!!\n");
                            ...
                            Ptr appData)
{
    ...
    if (NULL != i2cDrvObj)
    {
        ...
        if (!instObj->numOpens)
        {
            ...
            /* Clear STAT register */
            I2C_WRITE_REG_WORD(0xFFFFu, &instObj->i2cRegs->I2C_IRQSTATUS);
            ...
            if (FVID2_SOK == retVal)
            {
                ...
                /* Read it again */
                reg_val = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS);
            }
            else
            {
                        ...
                        PSP_I2cXferParams *i2cXferParams)
{
    ...
    if (FVID2_SOK == retVal)
    {
        ...
        if (FVID2_SOK == retVal)
        {
            ...
            {
                retVal = Vps_i2cTransferMsgPoll(i2cDrvObj, i2cXferParams);
            }
            else
            {
                retVal = Vps_i2cTransferMsgInt(i2cDrvObj, i2cXferParams);
            }
        }
...
int PSP_i2cProbe (UInt32 instanceId, UInt8 slaveAddr)
{
    ...
    reg_val = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQENABLE_SET);
    I2C_WRITE_REG_WORD(0xFFFFu, &instObj->i2cRegs->I2C_IRQENABLE_CLR);
...
static int wait_for_bb (UInt32 instanceId)
{
    ...
    while ((stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS) & I2C_STAT_BB)
        && timeout--)
    {
        I2C_WRITE_REG_WORD(stat, &instObj->i2cRegs->I2C_IRQSTATUS);
        udelay (I2C_DELAY_SMALL );
    }

    if (timeout <= 0)
    {
        Vps_printf (" I2C: timed out in wait_for_bb: I2C_IRQSTATUS=%x !!!\n",
            I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS));
...
static void flush_fifo(UInt32 instanceId)
{
     ...
     */
    while(1){
        stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS);
        if(stat == I2C_STAT_RRDY){
            I2C_READ_REG_BYTE(&instObj->i2cRegs->I2C_DATA);
            I2C_WRITE_REG_WORD(I2C_STAT_RRDY, &instObj->i2cRegs->I2C_IRQSTATUS);
            udelay(I2C_DELAY_SMALL );
        }else
            break;
    }
...
static UInt16 wait_for_pin (UInt32 instanceId)
{
    ...
    {
        udelay (I2C_DELAY_SMALL );
        status = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS);
    } while (!(status &
    ...
    if (timeout <= 0)
    {
        printf ("timed out in wait_for_pin: I2C_IRQSTATUS=%x\n",
                I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS));
        I2C_WRITE_REG_WORD(0xFFFF, &instObj->i2cRegs->I2C_IRQSTATUS);
    }
                        ...
                        PSP_I2cXferParams *i2cXferParams)
{
    ...
    if (i2cXferParams->flags & I2C_WRITE)
    {
        ...
        udelay (I2C_DELAY_BIG );
        if (I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS) & I2C_STAT_NACK)
        {
            i2c_error = 1;
        }
    }
                        ...
                        PSP_I2cXferParams *i2cXferParams)
{
    ...
    cookie = Hwi_disable();
    semOrder[semindex] = 0;
    semTimeOrder[semindex++] = Clock_getTicks();
    numOfSemPends++;
    Hwi_restore(cookie);
#endif /*I2C_DEBUG*/
...
static void i2c_isr(UArg arg)
{
    ...
    isrIndex++;
    stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS_RAW);
    isrstat[index++] = stat;
#endif /*I2C_DEBUG*/

    /* check enabled interrupt bits */
    bits = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQENABLE_SET);

    /*handle Isr only when io is in pending state*/
    if(instObj->ioPendingState == TRUE)
    {
            //isrindexloop = 0;
           while ((stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS)) & bits)
        {

#ifdef I2C_DEBUG
            isrStatusloop[isrindexloop++] = stat;
#endif /*I2C_DEBUG*/
            ...
            I2C_WRITE_REG_WORD(w, &instObj->i2cRegs->I2C_IRQSTATUS);
            ...
            if (stat & (I2C_STAT_ARDY | I2C_STAT_NACK |
                        I2C_STAT_AL))
            {
                w = stat & (I2C_STAT_RRDY | I2C_STAT_RDR |
                    I2C_STAT_XRDY | I2C_STAT_XDR);
...
#ifdef I2C_DEBUG
                        if (stat & I2C_STAT_RRDY)
                            Vps_printf (" I2C: RRDY IRQ while no data requested !!!\n");
                        if (stat & I2C_STAT_RDR)
                            Vps_printf (" I2C: RDR IRQ while no data requested !!!\n");
#endif
                        break;
                    }
                }
                I2C_WRITE_REG_WORD(stat & (I2C_STAT_RRDY | I2C_STAT_RDR),
                    &instObj->i2cRegs->I2C_IRQSTATUS);
                continue;
            }
                ...
                while (num_bytes)
                {
                    ...
                    else
                    {
                        ...
                        if (stat & I2C_STAT_XRDY)
                            Vps_printf (" I2C: XRDY IRQ while no data to send !!!\n");
...
#ifdef I2C_DEBUG
            isrStatus[1][isrIndex] =
                I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS_RAW);
#endif /*I2C_DEBUG*/

        stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS_RAW);
        while(stat & I2C_STAT_RRDY)
        {
            w = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_DATA);
            I2C_WRITE_REG_WORD(stat,
                     &instObj->i2cRegs->I2C_IRQSTATUS);
            stat = I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS_RAW);
        }
           I2C_WRITE_REG_WORD(0xFF,
                     &instObj->i2cRegs->I2C_IRQSTATUS);
#ifdef I2C_DEBUG
        isrStatus[2][isrIndex] =
            I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_IRQSTATUS_RAW);
#endif /*I2C_DEBUG*/
...
static int i2c_init (UInt32 instanceId)
{
    ...
    * reset done bit gets set. */
    ...
    while ((!(I2C_READ_REG_WORD(&instObj->i2cRegs->I2C_SYSS) & SYSS_RESETDONE_MASK))
        && delay)
    {
        delay--;
        udelay (I2C_DELAY_SMALL);
