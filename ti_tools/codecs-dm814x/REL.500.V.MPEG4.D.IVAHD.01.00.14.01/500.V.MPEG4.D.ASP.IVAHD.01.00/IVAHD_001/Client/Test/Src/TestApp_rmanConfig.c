 ...
 * @author: Ashish SIngh
 *
 * @version 0.0 (August 2011) : Base version
 *
 * @version 0.1 (Sep  2011) : Updated the code with tiler memory allocation for
 *                            optional deblocking on
 * @version 0.2 (July 2012) : Support for only I frames decoding added 
 *                            [Mahantesh]
 ******************************************************************************
*/
...
#define TILER_BUFF_IDX (MAX_BUFF_ELEMENTS<<1)
...
/* IMPEG4VDEC_Handle: Algorithm specific handle                               */
/*--------------------------------------------------------------------------*/
 ...
 *******************************************************************************
 *
 *@fn   MPEG4VDEC_TI_deactivateResource
 *
 ...
 *******************************************************************************
*/
extern IRES_Status MPEG4VDEC_TI_deactivateResource(IALG_Handle handle,
                                                  IRES_Handle resourceHandle);

/**
 *******************************************************************************
 *
 *@fn   MPEG4VDEC_TI_activateResource
 *
 ...
 *******************************************************************************
*/
extern IRES_Status MPEG4VDEC_TI_activateResource(IALG_Handle handle,
                                                IRES_Handle resourceHandle);
/**
 *******************************************************************************
 *
...
void HDVICP_Acquire(IALG_Handle handle, IRES_HDVICP2_Handle iresHandle,
                    ...
                    XDAS_UInt32 *configurationId,Int lateAcquireArg
                   )
{
  XDAS_Int32 ID = 0;
  ...
  /*--------------------------------------------------------------------------*/
  if((IALG_Handle)mpeg4decHandle != handle)
  {
  ...
  /*----------------------------------------------------------------------------*/
  /* Complate memory space of IVAHD is getting corrupted at each frame decoding */
  /* so need to set codec type as DIFFERENTCODEC so that codec should be able   */
  /* load code / data in each process call from DDR to its own internal memory  */
  /*----------------------------------------------------------------------------*/
#ifdef MEM_CORRUPTION_TEST
   *reloadHDVICP = DIFFERENTCODEC;
#endif
...
XDAS_UInt32 HDVICP_Reset(IALG_Handle handle, IRES_HDVICP2_Handle iresHandle)
{
  ...
  unsigned int RM_IVAHD0_RSTST = (RM_IVAHD0_RSTCTRL + 4); //(PRM_IVAHD0 + 0x14)
...
XDAS_Int8 RMAN_AssignResources(IALG_Handle handle)
{
  ...
  XDAS_UInt32 lumaBuffIdx = 0;
  XDAS_UInt32 ChromaBuffIdx = 1;
  /*--------------------------------------------------------------------------*/
  ...
  /*--------------------------------------------------------------------------*/
  numResources       = MPEG4VDEC_TI_IRES.numResourceDescriptors(handle);
  ...
  /*--------------------------------------------------------------------------*/
  retVal = MPEG4VDEC_TI_IRES.getResourceDescriptors(handle, resourceDescriptor);
  ...
  /* hadle                                                                    */
  /*--------------------------------------------------------------------------*/
  resourceDescriptor[0].handle = (IRES_Obj *)&MPEG4IresHdvicp2_handle;

  hdvicp2ProtocolArgs  =
             (IRES_HDVICP2_ProtocolArgs*)resourceDescriptor[0].protocolArgs;

  MPEG4IresHdvicp2_handle.id = hdvicp2ProtocolArgs->id;
  ...
  for(idx = 1 ; idx<numResources ; idx++)
  {
    ...
    /* Descriptor.                                                            */
    /*------------------------------------------------------------------------*/
    resourceDescriptor[idx].handle = (IRES_Obj *)&iresTiledMemoryObjects[idx-1];
    ...
    /* Check if codec has asked about 2D memeory which is mostly incase of    */
    /* filtering , codec may ask for extra buffer else it will be always 1D   */
    /* Buffer                                                                 */
    /*------------------------------------------------------------------------*/
    if(tiledmemProtocolArgs->sizeDim1)
    {
	  /*-----------------------------------------------------------------*/
	  /* incase of tiler memory allocation requirement                   */
	  /* allocate memory for Luma buffers                                */
	  /* then allocate the memory for chroma buffers                     */
	  /* else allocte the raw memory                                     */
      /* Buffer indexes will change when decodeOnlyIntraFrames = 1       */
	  /*-----------------------------------------------------------------*/
	  if(appControlPrms.tilerEnable)
	  {
        if(params.decodeOnlyIntraFrames == 0)
        {
            if(idx < IMPEG4VDEC_CHROMARECON_DATA_BUF0)
            {
                iresTiledMemoryObjects[idx-1].memoryBaseAddress =
                            (Void *)tilerParams.tiledBufferAddr[
                            TILER_BUFF_IDX + lumaBuffIdx];
                lumaBuffIdx += 2;
                iresTiledMemoryObjects[idx-1].accessUnit  = IRES_TILEDMEMORY_8BIT;
            } /*if(idx < IMPEG4VDEC_CHROMARECON_DATA_BUF0)*/
            else
            {
                iresTiledMemoryObjects[idx-1].memoryBaseAddress =
                            (Void *)tilerParams.tiledBufferAddr[
                            TILER_BUFF_IDX + ChromaBuffIdx];
                ChromaBuffIdx += 2;
                /*----------------------------------------------------------*/
                /* Update the tiler memory type 8bit chroma or 16 bit Chroma*/
                /*----------------------------------------------------------*/
                if(!appControlPrms.chromaTilerMode)
                {
                  iresTiledMemoryObjects[idx-1].accessUnit  = IRES_TILEDMEMORY_16BIT;
                }
                else
                {
                  iresTiledMemoryObjects[idx-1].accessUnit  = IRES_TILEDMEMORY_8BIT;
        ...
        {
            if(idx < IMPEG4VDEC_CHROMARECON_DATA_BUF0_DECODE_ONLY_I_FRAMES)
            {
                iresTiledMemoryObjects[idx-1].memoryBaseAddress =
                            (Void *)tilerParams.tiledBufferAddr[
                            TILER_BUFF_IDX + lumaBuffIdx];
                lumaBuffIdx += 2;
                iresTiledMemoryObjects[idx-1].accessUnit  = 
        ...
        iresTiledMemoryObjects[idx-1].isTiledMemory = 1;
      } /*if(appControlPrms.tilerEnable)*/
      else
      {
        iresTiledMemoryObjects[idx-1].memoryBaseAddress =
                          (Void *)my_Memalign( tiledmemProtocolArgs->alignment,
                           (tiledmemProtocolArgs->sizeDim0 *
                            tiledmemProtocolArgs->sizeDim1 *
                          sizeof(XDAS_UInt8)));
        iresTiledMemoryObjects[idx-1].isTiledMemory = 0;
        iresTiledMemoryObjects[idx-1].accessUnit  = IRES_TILEDMEMORY_RAW;
	  } /*if(appControlPrms.tilerEnable)*/
    }
    else
    {
...
#ifdef MEM_CORRUPTION_TEST
   {
    ...
    if(iresTiledMemoryObjects[idx-1].memoryBaseAddress != NULL)
    {
      memset ((Void*)(iresTiledMemoryObjects[idx-1].memoryBaseAddress), word,
              (tiledmemProtocolArgs->sizeDim0 * sizeof(XDAS_UInt8)));
    }
   }
#endif
  ...
  /*--------------------------------------------------------------------------*/
  retVal |= MPEG4VDEC_TI_IRES.initResources(handle, &resourceDescriptor[0],
                                           yieldFxn, yieldArgs
                                          );
  /*--------------------------------------------------------------------------*/
...
void RMAN_FreeResources(IALG_Handle handle)
{
  ...
  MPEG4VDEC_TI_deactivateResource(handle,
        (IRES_Handle)&MPEG4IresHdvicp2_handle);
  /*--------------------------------------------------------------------------*/
...
{
  MPEG4VDEC_TI_activateResource(handle,
        (IRES_Handle)&MPEG4IresHdvicp2_handle);
