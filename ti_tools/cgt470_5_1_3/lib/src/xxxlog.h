...
static const _DATA_ACCESS FTYPE p[] = {	/* 7th-order, |x| < 3-sqrt(8) (100.6%) */
#else
static const FTYPE p[] = {	/* 7th-order, |x| < 3-sqrt(8) (100.6%) */
  ...
  #define POLY(x)	((((((p[0] * x + p[1]) * x + p[2]) * x + p[3]) * x \
		+ p[4]) * x + p[5]) * x + p[6])
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE p[] = {	/* 15th-order, |x| < 3-sqrt(8) (100.6%) */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE p[] = {	/* 17th-order, |x| < 3-sqrt(8) (100.0%) */
#else
static const FTYPE p[] = {	/* 17th-order, |x| < 3-sqrt(8) (100.0%) */
  ...
  #define POLY(x)	FNAME(Poly)(x, p, sizeof (p) / sizeof (p[0]) - 1)
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE p[] = {	/* 31st-order, |x| < 3-sqrt(8) (100.0%) */
#else
static const FTYPE p[] = {	/* 31st-order, |x| < 3-sqrt(8) (100.0%) */
 ...
 #endif /* FBITS < 24 */
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE loge2hi = (FTYPE)(5814539.0 / 8388608.0);
static const _DATA_ACCESS FTYPE loge2lo = FLIT(1.1730463525082348212145817656807550e-7);

static const _DATA_ACCESS FTYPE logx2hi = (FTYPE)(2525222.0 / 8388608.0);
static const _DATA_ACCESS FTYPE logx2lo = FLIT(7.5085978265526238894724493026768190e-8);

static const _DATA_ACCESS FTYPE logxehi = (FTYPE)(3643126.0 / 8388608.0);
static const _DATA_ACCESS FTYPE logxelo = FLIT(1.9699272335463628918916605082294398e-8);
static const _DATA_ACCESS FTYPE logxe   = FLIT(0.43429448190325182765112891891660507);

static const _DATA_ACCESS FTYPE log2ehi = (FTYPE)(12102203.0 / 8388608.0);
static const _DATA_ACCESS FTYPE log2elo = FLIT(1.9259629911266174681001892137426646e-8);
static const _DATA_ACCESS FTYPE log2e   = FLIT(1.4426950408889634073599246810018922);
...
#endif /* defined(__TI_COMPILER_VERSION__) */
FTYPE FNAME(Log)(FTYPE x, int baseflag)
	{	/* compute ln(x) */
	short xexp;
