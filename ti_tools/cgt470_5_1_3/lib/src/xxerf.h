...
static const  _DATA_ACCESS FTYPE s[] = {	/* 3/3, |x| < 1.523 */
#else
...
static const FTYPE c[] = {
  ...
  #define NPOLY(x)	((((((s[0] * x + s[1]) * x + s[2]) * x + s[3]) * x \
		+ s[4]) * x + s[5]) * x + s[6])
#if defined(__TI_COMPILER_VERSION__)
static const  _DATA_ACCESS FTYPE s[] = {	/* 6/6, |x| < 1.523 */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const  _DATA_ACCESS FTYPE s[] = {	/* 7/7, |x| < 1.523 */
#else
...
static const FTYPE c[] = {
  ...
  #define NPOLY(x)	FNAME(Poly)(x, s, sizeof (s) / sizeof (s[0]) - 1)
#if defined(__TI_COMPILER_VERSION__)
static const  _DATA_ACCESS FTYPE s[] = {	/* 11/11, |x| < 1.523 */
#else
