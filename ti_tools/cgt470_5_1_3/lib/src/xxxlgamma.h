...
static const _DATA_ACCESS FTYPE c[] = {	/* Stirling fixup order 0, Gamma_big < x */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* Stirling fixup order 2, Gamma_big < x */
#else
static const FTYPE c[] = {	/* Stirling fixup order 2, Gamma_big < x */
  ...
  #define POLY(xinv, w)	(((((c[0] * w + c[1]) * w + c[2]) * w + c[3]) * w \
	+ c[4]) * xinv)
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* Stirling fixup order 4, Gamma_big < x */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE c[] = {	/* Stirling fixup order 10, Gamma_big < x */
#else
...
#if defined(__TI_COMPILER_VERSION__)
static const _DATA_ACCESS FTYPE half_lntwopi = FLIT(0.91893853320467274178032973640561765);
#else
