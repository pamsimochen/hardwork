...
static int i2c_read_bytes(u8 devaddr, u8 *aBuf, u8 aLen, u8 *dBuf, u8 dLen)
{
	int i2c_error = 0, i;
	u16 status, temp;
...
#if defined(CONFIG_OMAP243X) || defined(CONFIG_OMAP34XX) || defined(CONFIG_TI81XX)
			/* send out 1 byte */
			writeb(aBuf[i], I2C_DATA);
			writew(I2C_STAT_XRDY, I2C_STAT);

#else
			/* send out two bytes */
			if(aLen - i >= 2) {
				writew((aBuf[i+1] << 8) + aBuf[i], I2C_DATA);
				i++;
			} else {
				writew(aBuf[i], I2C_DATA);
			}
#endif
			if(i == aLen - 1) {
				/* must have enough delay to allow BB bit to go low */
...
int i2c_write_bytes(u8 devaddr, u8 *aBuf, u8 aLen, u8 *dBuf, u8 dLen)
{
	int i2c_error = 0, i;
	u16 status, stat;
...
#if defined(CONFIG_OMAP243X) || defined(CONFIG_OMAP34XX) || defined(CONFIG_TI81XX)
			/* send out 1 byte */
			writeb(dBuf[i], I2C_DATA);
			writew(I2C_STAT_XRDY, I2C_STAT);

#else
			/* send out two bytes */
			if(dLen - i >= 2) {
				writew((dBuf[i+1] << 8) + dBuf[i], I2C_DATA);
				i++;
			} else
				writew(dBuf[i], I2C_DATA);
#endif
			if(i == dLen - 1) {
				/* must have enough delay to allow BB bit to go low */
				udelay(50000);
...
int i2c_read(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	if(alen > 2) {
		...
		nI2cReg16 = cpu_to_be16(addr);
		if(i2c_read_bytes(chip, (u8*)&nI2cReg16, alen, buffer, len)) {
...
int i2c_write(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	if(alen > 2) {
		...
		nI2cReg16 = cpu_to_be16(addr);
		if(i2c_write_bytes(chip, (u8 *) &nI2cReg16, alen, buffer, len)) {
