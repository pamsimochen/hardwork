...
{
//#ifndef (CONFIG_TI814X_OPTI_CONFIG | ECC_BCH8_OFF)
#if 1
	int i;
	struct nand_chip *chip = mtd->priv;
	u16 *p = (u16 *) buf;
			    ...
			    struct mtd_oob_ops *ops)
{
	...
	while(1) {
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if(!sndcmd && !NAND_CANAUTOINCR(chip) && (readlen > mtd->writesize)) {
				if((page & blkcheck) != blkcheck) {
					chip->cmdfunc(mtd, NAND_CMD_READ_CACHE_START, -1, -1);
					read_cache_flag = 1; /* flags that 31h command has been sent */
				}
#endif				
			}
...
#else
			if(!sndcmd && !NAND_CANAUTOINCR(chip)) {
				if((readlen <= mtd->writesize) || ((page & blkcheck) == blkcheck)) {
					if(read_cache_flag) {
						chip->cmdfunc(mtd, NAND_CMD_READ_CACHE_STOP, -1, -1);
						read_cache_flag = 0;
					}
				}
				if((page & blkcheck) == 0)
