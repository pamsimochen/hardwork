...
/*
 * Syntax:
 *	i2c write {i2c_chip} {devaddr}{.0, .1, .2} {value}
 */

static int do_i2c_write ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	u_char	chip;
	uint	devaddr, alen, value;
	...
	if(alen > 4){
		printf("Not support for address length %d bytes.\n", alen);
		return 1;
	}
	value <<= ((4 - alen) * 8);
	value = cpu_to_be32(value);

	if (i2c_write(chip, devaddr, alen, (uchar *)&value, 1) != 0) {
...
static cmd_tbl_t cmd_i2c_sub[] = {
	...
	U_BOOT_CMD_MKENT(write, 4, 1, do_i2c_write, "", ""),
#if defined(CONFIG_CMD_SDRAM)
...
U_BOOT_CMD(
	...
	"i2c write chip address[.0, .1, .2] value - write a value to I2C device\n"
#if defined(CONFIG_CMD_SDRAM)
