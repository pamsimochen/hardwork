...
int alarm_main_loop(int qid, int audQid, SemHandl_t hAlarmSem)
{
	...
	while(1){
		...
		if( msg_size < 0 ){
            ...
            // while dmva event deletion is underway,and if dmva event recordings are
            // happening sometimes msgrcv is failing with the errno = ERESTARTNOHAND(514)
            // action taken is to ignore it.
			if(errno == 514)
			{
				printf("\nerrno = %d\n",errno);
			    continue;
