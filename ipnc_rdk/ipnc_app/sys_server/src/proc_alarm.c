...
/**
* @brief Set Dmva Event Type.
*/
void SetDmvaEvtType(int evtType)
{
    gDmvaEvtType = evtType;
}

/**
* @brief Get Dmva Event Type.
*/
int GetDmvaEvtType()
{
    return (gDmvaEvtType);
...
void *ProcAlarmThread(void *arg)
{
	...
	switch(pEvironment->msg_buf.event){
		...
		case ALARM_EVENT_DMVA_START:
		{
			DBG("ALARM_EVENT_DMVA_START \n");
			if(pSysInfo->dmva_config.dmvaAppEventRecordingVAME_sys & 0x8)
			{
				if((pSysInfo->sdcard_config.sdinsert == 3) && (gbSD_WriteProtect == 0))
				{
#ifdef AVI_BIG_FILE
					AviRun(AVI_MODE2, AVI_TO_SD | AVI_FOR_DMVA);
#else
					AviRun(AVI_MODE3, AVI_TO_SD | AVI_FOR_DMVA);
#endif
					SemWait(pEvironment->hAlarmSem);
					gSchAviRun = 1;
					SetDmvaEvtType(pEvironment->msg_buf.serial);
					SemRelease(pEvironment->hAlarmSem);
				}
			}			
			break;
		}
		case ALARM_EVENT_DMVA_STOP:
		{
			DBG("ALARM_EVENT_DMVA_STOP \n");
			if(pSysInfo->sdcard_config.sdinsert == 3)
			{
				SemWait(pEvironment->hAlarmSem);
				if((gbSD_WriteProtect == 0) && (gSchAviRun == 1))
				{
					AviStop();
					gSchAviRun = 0;
				}
