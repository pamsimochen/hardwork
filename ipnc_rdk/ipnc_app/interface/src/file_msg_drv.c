...
int fSetVstabValue(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.vsEnable, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetLdcValue(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.ldcEnable, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetVnfValue(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.vnfEnable, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetVnfMode(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.vnfMode, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetVnfStrength(unsigned char value)
{
	...
	memcpy(&pSysInfo->advanceMode.vnfStrength, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetDynRange(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.dynRange, &value, sizeof(value));
	return SetSysInfo(0);
}
int fSetDynRangeStrength(unsigned char value)
{
	...
	if(pSysInfo == NULL)
		return -1;
	memcpy(&pSysInfo->advanceMode.dynRangeStrength, &value, sizeof(value));
	return SetSysInfo(0);
}
...
int faddaudioencoderconfiguration(Add_AudioEncoder_Configuration_tmp *addaudioencoder)
{
	...
	{
		if(strcmp(addaudioencoder->Add_AudioEncoder_in.AEtoken_t, pSysInfo->Oprofile[i].AAEC.AEtoken) == 0)
		{
...
int fSetDmvaSysLog(DmvaLogEntry_t* value)
{
	...
	if(pSysInfo == NULL || value == NULL)
		return -1;
//	memcpy(&pSysInfo->tCurDmvaLog, value, sizeof(DmvaLogEntry_t));
//	DBG("dmva event:%s\n", pSysInfo->tCurDmvaLog.eventListEventDetails_sys);
//	DBG("dmva time:%s\n", asctime(&pSysInfo->tCurDmvaLog.time));
...
int fSetDmvaEventListTimeStamp(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->tCurDmvaLog.eventListTimeStamp_sys)==0)
	    return -2;
...
int fSetDmvaEventListEventDetails(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->tCurDmvaLog.eventListEventDetails_sys)==0)
	    return -2;
...
int fSetDmvaEnvironmentProfileDesc(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaEnvironmentProfileDesc_sys)==0)
	    return -2;
...
int fSetDmvaSMETAZone1_Label(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaSMETAZone_1_Label_sys)==0)
	    return -2;
...
int fSetDmvaIMDZone1_Label(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaIMDZone_1_Label_sys)==0)
	    return -2;
...
int fSetDmvaIMDZone2_Label(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaIMDZone_2_Label_sys)==0)
	    return -2;
...
int fSetDmvaIMDZone3_Label(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaIMDZone_3_Label_sys)==0)
	    return -2;
...
int fSetDmvaIMDZone4_Label(void *buf, int len)
{
	...
	if(strcmp(buf,pSysInfo->dmva_config.dmvaIMDZone_4_Label_sys)==0)
	    return -2;
...
static int dmvaCheckOverlap(SysInfo *pSysInfo,DMVA_schedule_t *pDmvaSchedule)
{
    ...
    for(i = 0;i < DMVA_SCH_MAX_NUM_SCHEDULE;i ++)
    {
        if((strcmp(pSysInfo->dmva_config.dmvaScheduleMgr.dmvaSchedule[i].ruleLabel,pDmvaSchedule->ruleLabel) != 0) &&
...
{
    Dmva_save_load_rules_t *pLoadSave;
...
int fSetDmvaSchDelRule(void *buf, int len)
{
    ...
    for(i = 0;i < DMVA_SCH_MAX_NUM_SCHEDULE;i ++)
    {
        if((strcmp(ruleLabel,pSysInfo->dmva_config.dmvaScheduleMgr.dmvaSchedule[i].ruleLabel) == 0) &&
           (pSysInfo->dmva_config.dmvaScheduleMgr.dmvaSchedule[i].isActive == 1))
        {
