...
#define GIO_MAX_ID	(128)

#define GIO_DEV_PATH	"/sys/class/gpio/"
...
*/
int ipnc_gio_getname( char *pName, int gio_id )
{
	...
	sprintf( pName, GIO_DEV_PATH"gpio%d/value", gio_id );
...
int ipnc_gio_read( int gio_id )
{
	...
	if( ipnc_gio_getname(dev_name,gio_id) < 0 )
	{
...
*@retval 0  success
*/
int ipnc_gio_write( int gio_id , int IsOn)
{
#ifdef IPNC_GPIO_CFG
	int 	fd_gio = 0;
	char	data[5];
