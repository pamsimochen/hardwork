...
typedef enum EncDec_ResolutionClass {
    ...
    UTILS_ENCDEC_RESOLUTION_CLASS_1080P = UTILS_ENCDEC_RESOLUTION_CLASS_FIRST,
    UTILS_ENCDEC_RESOLUTION_CLASS_720P,
    UTILS_ENCDEC_RESOLUTION_CLASS_960P,
...
#define UTILS_ENCDEC_RESOLUTION_CLASS_960P_WIDTH                          (1280)
#define UTILS_ENCDEC_RESOLUTION_CLASS_960P_HEIGHT                          (960)
...
static Uint32 Timestamp[48];	// for playback frames

static Uint32 Get_timeStamp(Uint32 chId) {
		Uint32 temp = Timestamp[chId];
		Timestamp[chId] += 33;	// avsync hack
	//	if (chId == 0)
		//	OSA_printf("\n#####\nIPC-BITS-OUT : timestamp set[%d] for ch %d @ time = %d\n\n", temp, chId, OSA_getCurTimeInMsec());
		return temp;
}
...
#define UTILS_ENCDEC_GET_BITBUF_SIZE(width,height)   ( (  (width) * (height)  ) )
                                                           ...
                                                           UInt32 height)
{
    ...
    {
        if ((width > UTILS_ENCDEC_RESOLUTION_CLASS_960P_WIDTH) ||
            (height > UTILS_ENCDEC_RESOLUTION_CLASS_960P_HEIGHT))
        {
            resClass = UTILS_ENCDEC_RESOLUTION_CLASS_1080P;
        } else {
            resClass = UTILS_ENCDEC_RESOLUTION_CLASS_960P;
        }
    }
    else
    {
    ...
    switch (resClass)
    {
        ...
        case UTILS_ENCDEC_RESOLUTION_CLASS_720P:
            ...
            *pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_720P_HEIGHT;
            break;
        case UTILS_ENCDEC_RESOLUTION_CLASS_960P:
            *pWidth = UTILS_ENCDEC_RESOLUTION_CLASS_960P_WIDTH;
            *pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_960P_HEIGHT;
            break;
                                         ...
                                         UInt32 minBufSize)
{
    ...
    if (i < pObj->outQueInfo.allocPoolInfo.numPools)
    {
        allocPoolID = i;
    }
    return (allocPoolID);
