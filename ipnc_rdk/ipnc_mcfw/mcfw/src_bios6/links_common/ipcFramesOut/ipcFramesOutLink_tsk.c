...
Void IpcFramesOutLink_setOutQueInfo(IpcFramesOutLink_Obj *pObj)
{
    ...
    /* split input channels equally across multiple output queues */
    chPerOutQueue = (pInQueInfo->numCh / pObj->createArgs.baseCreateParams.numOutQue);
    ...
    {
        /* get output channel ID, output queue ID, from input CH ID */
        outChId = inChId % chPerOutQueue;
        queueId = inChId / chPerOutQueue;

        /* confirm output que ID is within limits */
        UTILS_assert(queueId < pObj->info.numQue);

        /* set channels per output que */
        pObj->info.queInfo[queueId].numCh = chPerOutQueue;
                                      ...
                                      FVID2_FrameList * pFrameBufList)
{
    ...
    if (pFrameBufList->numFrames)
    {
        ...
        chPerQueue = (pObj->numCh / pObj->createArgs.baseCreateParams.numOutQue);
        ...
        for (i = 0; i < pFrameBufList->numFrames;i++)
        {
            ...
            pFrameBuf->channelNum += (queId * chPerQueue);
        }
